"""
Main AI Trading Bot worker class.
"""
...
from ai_trading_bot.configuration import Configuration
from ai_trading_bot.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from ai_trading_bot.enums import RPCMessageType, State
from ai_trading_bot.exceptions import OperationalException, TemporaryError
from ai_trading_bot.exchange import timeframe_to_next_date
...
class Worker:
    """
    AI Trading Botbot worker class
    """
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.ai-trading-bot = TradingBot(self._config)
    ...
    def _worker(self, old_state: State | None) -> State:
        ...
        """
        state = self.ai_trading_bot.state

        # Log state transition
        if state != old_state:
            if old_state != State.RELOAD_CONFIG:
                self.ai_trading_bot.notify_status(f"{state.name.lower()}")
            ...
            if state in (State.RUNNING, State.PAUSED) and old_state not in (
                ...
                self.ai_trading_bot.startup()

            if state == State.STOPPED:
                self.ai_trading_bot.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.ai_trading_bot.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.ai_trading_bot.process_stopped()

    def _process_running(self) -> None:
        try:
            self.ai_trading_bot.process()
            ...
            self.ai_trading_bot.notify_status(
                f"*OperationalException:*\n```\n{tb}```\n {hint}", msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception("OperationalException. Stopping trader ...")
            self.ai_trading_bot.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current trading_bot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current ai-trading-bot modules
        self.ai_trading_bot.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.ai_trading_bot.notify_status(f"{State(self.ai_trading_bot.state)} after config reloaded")
    ...
    def exit(self) -> None:
        ...
        if self.ai_trading_bot:
            self.ai_trading_bot.notify_status("process died")
            self.ai_trading_bot.cleanup()
