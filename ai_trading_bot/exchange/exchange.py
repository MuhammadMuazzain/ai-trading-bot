...
from ai_trading_bot.constants import (
...
from ai_trading_bot.data.converter import (
...
from ai_trading_bot.exchange.common import (
    API_FETCH_ORDER_RETRY_COUNT,
    remove_exchange_credentials,
    retrier,
    retrier_async,
)
from ai_trading_bot.exchange.exchange_types import (
    CcxtBalances,
    CcxtOrder,
    CcxtPosition,
    FtHas,
...
from ai_trading_bot.exchange.exchange_ws import ExchangeWS
from ai_trading_bot.misc import (
    chunks,
    deep_merge_dicts,
    file_dump_json,
    file_load_json,
...
from ai_trading_bot.util import dt_from_ts, dt_now
from ai_trading_bot.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
...
class Exchange:
    ...
    def market_is_tradable(self, market: dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by AI Trading Bot.
    ...
    def validate_timeframes(self, timeframe: str | None) -> None:
        ...
        if (
            ...
            raise ConfigurationError("Timeframes < 1m are currently not supported by AI Trading Bot.")
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if ai-trading-bot can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by ai-trading-bot on this exchange
        """
        if trading_mode != TradingMode.SPOT and (
            ...
            raise OperationalException(
                f"AI Trading Bot does not support {mm_value} {trading_mode} on {self.name}"
            )
    ...
    def fetch_dry_run_order(self, order_id) -> CcxtOrder:
        ...
        try:
            ...
            from ai_trading_bot.persistence import Order
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("AI Trading Bot does not yet support inverse contracts")
            ...
            raise OperationalException(
                "AI Trading Bot only supports isolated futures for leverage trading"
            )
