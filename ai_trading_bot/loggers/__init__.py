...
from ai_trading_bot.loggers.buffering_handler import FTBufferingHandler
from ai_trading_bot.loggers.ft_rich_handler import FtRichHandler
from ai_trading_bot.loggers.rich_console import get_rich_console


# from ai_trading_bot.loggers.std_err_stream_handler import FTStdErrStreamHandler
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "ai-trading-bot": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config(config: Config) -> dict[str, Any]:
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "ai_trading_bot.loggers.ft_rich_handler.FtRichHandler":
            ...
            try:
                ...
                raise OperationalException(
                    ...
                    "directories. If you're running ai-trading-bot using docker, you see this error "
