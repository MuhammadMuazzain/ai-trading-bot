...
from ai_trading_bot.constants import CANCEL_REASON, DEFAULT_DATAFRAME_COLUMNS, Config
from ai_trading_bot.data.history import load_data
from ai_trading_bot.data.metrics import DrawDownResult, calculate_expectancy, calculate_max_drawdown
...
from ai_trading_bot.exceptions import ExchangeError, PricingError
from ai_trading_bot.exchange import Exchange, timeframe_to_minutes, timeframe_to_msecs
from ai_trading_bot.exchange.exchange_utils import price_to_precision
from ai_trading_bot.loggers import bufferHandler
from ai_trading_bot.persistence import CustomDataWrapper, KeyStoreKeys, KeyValueStore, PairLocks, Trade
from ai_trading_bot.persistence.models import PairLock
from ai_trading_bot.plugins.pairlist.pairlist_helpers import expand_pairlist
from ai_trading_bot.rpc.fiat_convert import CryptoToFiatConverter
from ai_trading_bot.rpc.rpc_types import RPCSendMsg
from ai_trading_bot.util import (
    decimals_per_coin,
    dt_from_ts,
    dt_humanize_delta,
    dt_now,
...
from ai_trading_bot.wallets import PositionWallet, Wallet
...
class RPC:
    ...
    if TYPE_CHECKING:
        ...
        _ai_trading_bot: TradingBot

    def __init__(self, ai_trading_bot) -> None:
        """
        Initializes all enabled rpc modules
        :param ai_trading_bot: Instance of a ai-trading-bot bot
        :return: None
        """
        self._ai-trading-bot = ai_trading_bot
        self._config: Config = ai_trading_bot.config
    ...
    def _rpc_show_config(
        ...
        val = {
            ...
            "bot_name": config.get("bot_name", "ai-trading-bot"),
    ...
    def _rpc_trade_status(self, trade_ids: list[int] | None = None) -> list[dict[str, Any]]:
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.is_open:
                    try:
                        current_rate: float = self._ai_trading_bot.exchange.get_rate(
                ...
                if not isnan(current_profit_abs) and self._fiat_converter:
                    current_profit_fiat = self._fiat_converter.convert_amount(
                        current_profit_abs,
                        self._ai_trading_bot.config["stake_currency"],
                        self._ai_trading_bot.config["fiat_display_currency"],
                    )
    ...
    def _rpc_timeunit_profit(
        ...
        daily_stake = self._ai_trading_bot.wallets.get_total_stake_amount()
    ...
    def _rpc_trade_statistics(
        ...
        for trade in trades:
            ...
            if not trade.is_open:
                ...
                if len(trade.select_filled_orders(trade.entry_side)) == 0:
                    ...
                    continue
                try:
                    current_rate = self._ai_trading_bot.exchange.get_rate(
                        trade.pair, side="exit", is_short=trade.is_short, refresh=False
                    )
        ...
        starting_balance = self._ai_trading_bot.wallets.get_starting_balance()
    ...
    def __balance_get_est_stake(
        ...
        if coin == self._ai_trading_bot.exchange.get_proxy_coin():
            ...
            try:
                rate: float | None = self._ai_trading_bot.exchange.get_conversion_rate(
                    coin, stake_currency
                )
                if rate:
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> dict:
        ...
        self._ai_trading_bot.wallets.update(require_update=False)
        starting_capital = self._ai_trading_bot.wallets.get_starting_balance()
        starting_cap_fiat = (
            self._fiat_converter.convert_amount(
        ...
        coin: str
        balance: Wallet
        for coin, balance in self._ai_trading_bot.wallets.get_all_balances().items():
            if not balance.total and not balance.free:
                continue

            trade = (
                open_assets.get(coin, None)
                if self._ai_trading_bot.trading_mode != TradingMode.FUTURES
                else None
            )
            is_stake_currency = coin == self._ai_trading_bot.exchange.get_proxy_coin()
            is_bot_managed = is_stake_currency or trade is not None
            trade_amount = trade.amount if trade else 0
            if is_stake_currency:
                trade_amount = self._ai_trading_bot.wallets.get_available_stake_amount()
        ...
        for symbol, position in self._ai_trading_bot.wallets.get_all_positions().items():
        ...
        return {
            ...
            "note": "Simulated balances" if self._ai_trading_bot.config["dry_run"] else "",
        }

    def _rpc_start(self) -> dict[str, str]:
        """Handler for start"""
        if self._ai_trading_bot.state == State.RUNNING:
            return {"status": "already running"}

        self._ai_trading_bot.state = State.RUNNING
        return {"status": "starting trader ..."}

    def _rpc_stop(self) -> dict[str, str]:
        """Handler for stop"""
        if self._ai_trading_bot.state != State.STOPPED:
            self._ai_trading_bot.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> dict[str, str]:
        ...
        self._ai_trading_bot.state = State.RELOAD_CONFIG
    ...
    def _rpc_pause(self) -> dict[str, str]:
        ...
        """
        if self._ai_trading_bot.state == State.RUNNING:
            self._ai_trading_bot.state = State.PAUSED
    ...
    def _rpc_reload_trade_from_exchange(self, trade_id: int) -> dict[str, str]:
        ...
        self._ai_trading_bot.handle_onexchange_order(trade)
    ...
    def __exec_force_exit(
        ...
        for oo in trade.open_orders:
            ...
            order = self._ai_trading_bot.exchange.fetch_order(oo.order_id, trade.pair)

            if order["side"] == trade.entry_side:
                fully_canceled = self._ai_trading_bot.handle_cancel_enter(
            ...
            if order["side"] == trade.exit_side:
                ...
                self._ai_trading_bot.handle_cancel_exit(trade, order, oo, CANCEL_REASON["FORCE_EXIT"])
        ...
        if all(tocr["cancel_state"] is False for tocr in trade_entry_cancelation_registry):
            ...
            order_type = ordertype or self._ai_trading_bot.strategy.order_types.get(
                "force_exit", self._ai_trading_bot.strategy.order_types["exit"]
            )
            sub_amount: float | None = None
            if amount and amount < trade.amount:
                # Partial exit ...
                min_exit_stake = self._ai_trading_bot.exchange.get_min_pair_stake_amount(
            ...
            self._ai_trading_bot.execute_trade_exit(
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        if not self._ai_trading_bot.config.get("force_entry_enable", False):
            raise RPCException("Force_entry not enabled.")

        if self._ai_trading_bot.state != State.RUNNING:
            raise RPCException("trader is not running")

        if order_side == SignalDirection.SHORT and self._ai_trading_bot.trading_mode == TradingMode.SPOT:
            raise RPCException("Can't go short on Spot markets.")

        if pair not in self._ai_trading_bot.exchange.get_markets(tradable_only=True):
            raise RPCException("Symbol does not exist or market is not active.")
        # Check if pair quote currency equals to the stake currency.
        stake_currency = self._ai_trading_bot.config.get("stake_currency")
        if not self._ai_trading_bot.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
    ...
    def _rpc_force_entry(
        ...
        *,
        ...
        if trade:
            ...
            if not self._ai_trading_bot.strategy.position_adjustment_enable:
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = self._ai_trading_bot.wallets.get_trade_stake_amount(
                pair, self._config["max_open_trades"]
            )

        # execute buy
        if not order_type:
            order_type = self._ai_trading_bot.strategy.order_types.get(
                "force_entry", self._ai_trading_bot.strategy.order_types["entry"]
            )
        with self._ai_trading_bot._exit_lock:
            if self._ai_trading_bot.execute_entry(
                pair,
                stake_amount,
                price,
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        ...
        with self._ai_trading_bot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    order = self._ai_trading_bot.exchange.fetch_order(open_order.order_id, trade.pair)
                except ExchangeError as e:
                    logger.info(f"Cannot query order for {trade} due to {e}.", exc_info=True)
                    raise RPCException("Order not found.")
                self._ai_trading_bot.handle_cancel_order(
    ...
    def _rpc_delete(self, trade_id: int) -> dict[str, str | int]:
        ...
        with self._ai_trading_bot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    self._ai_trading_bot.exchange.cancel_order(open_order.order_id, trade.pair)
                    c_count += 1
            ...
            if (
                self._ai_trading_bot.strategy.order_types.get("stoploss_on_exchange")
                and trade.has_open_sl_orders
            ):
                for oslo in trade.open_sl_orders:
                    try:
                        self._ai_trading_bot.exchange.cancel_stoploss_order(oslo.order_id, trade.pair)
                        c_count += 1
                    except ExchangeError:
                        pass
    ...
    def _rpc_count(self) -> dict[str, float]:
        ...
        return {
            "current": len(trades),
            "max": (
                int(self._ai_trading_bot.config["max_open_trades"])
                if self._ai_trading_bot.config["max_open_trades"] != float("inf")
                else -1
            ),
    ...
    def _rpc_whitelist(self) -> dict:
        ...
        res = {
            "method": self._ai_trading_bot.pairlists.name_list,
            "length": len(self._ai_trading_bot.active_pair_whitelist),
            "whitelist": self._ai_trading_bot.active_pair_whitelist,
        }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: list[str]) -> dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._ai_trading_bot.pairlists.blacklist:
                self._ai_trading_bot.pairlists.blacklist.remove(pair)
            else:
    ...
    def _rpc_blacklist(self, add: list[str] | None = None) -> dict:
        ...
        if add:
            for pair in add:
                if pair not in self._ai_trading_bot.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], list(self._ai_trading_bot.exchange.get_markets().keys()))
                        self._ai_trading_bot.pairlists.blacklist.append(pair)
        ...
        res = {
            "method": self._ai_trading_bot.pairlists.name_list,
            "length": len(self._ai_trading_bot.pairlists.blacklist),
            "blacklist": self._ai_trading_bot.pairlists.blacklist,
            "blacklist_expanded": self._ai_trading_bot.pairlists.expanded_blacklist,
            "errors": errors,
        }
        return res
    ...
    def _rpc_get_logs(limit: int | None) -> dict[str, Any]:
        ...
        #       "ai_trading_bot.worker", "INFO", "Starting worker develop"]

        return {"log_count": len(records), "logs": records}

    def _rpc_edge(self) -> list[dict[str, Any]]:
        """Returns information related to Edge"""
        if not self._ai_trading_bot.edge:
            raise RPCException("Edge is not enabled.")
        return self._ai_trading_bot.edge.accepted_pairs()
    ...
    def _rpc_analysed_dataframe(
        ...
        return RPC._convert_dataframe_to_dict(
            self._ai_trading_bot.config["strategy"], pair, timeframe, _data, last_analyzed, selected_cols
        )
    ...
    def __rpc_analysed_dataframe_raw(
        ...
        """
        _data, last_analyzed = self._ai_trading_bot.dataprovider.get_analyzed_dataframe(pair, timeframe)
        _data = _data.copy()
    ...
    def _ws_all_analysed_dataframes(
        ...
        """
        timeframe = self._ai_trading_bot.config["timeframe"]
        candle_type = self._ai_trading_bot.config.get("candle_type_def", CandleType.SPOT)
    ...
    def _ws_request_analyzed_df(self, limit: int | None = None, pair: str | None = None):
        ...
        pairlist = [pair] if pair else self._ai_trading_bot.active_pair_whitelist

        return self._ws_all_analysed_dataframes(pairlist, limit)

    def _ws_request_whitelist(self):
        """Whitelist data for WebSocket"""
        return self._ai_trading_bot.active_pair_whitelist
    ...
    def _rpc_analysed_history_full(
        ...
        from ai_trading_bot.data.converter import trim_dataframe
        from ai_trading_bot.data.dataprovider import DataProvider
    ...
    def _rpc_plot_config(self) -> dict[str, Any]:
        if (
            self._ai_trading_bot.strategy.plot_config
            and "subplots" not in self._ai_trading_bot.strategy.plot_config
        ):
            self._ai_trading_bot.strategy.plot_config["subplots"] = {}
        return self._ai_trading_bot.strategy.plot_config
    ...
    def health(self) -> dict[str, str | int | None]:
        last_p = self._ai_trading_bot.last_process
    ...
    def _update_market_direction(self, direction: MarketDirection) -> None:
        self._ai_trading_bot.strategy.market_direction = direction

    def _get_market_direction(self) -> MarketDirection:
        return self._ai_trading_bot.strategy.market_direction
