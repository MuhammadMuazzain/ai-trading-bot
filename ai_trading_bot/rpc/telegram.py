...
from ai_trading_bot.constants import DUST_PER_COIN, Config
from ai_trading_bot.enums import MarketDirection, RPCMessageType, SignalDirection, TradingMode
from ai_trading_bot.exceptions import OperationalException
from ai_trading_bot.misc import chunks, plural
from ai_trading_bot.persistence import Trade
from ai_trading_bot.rpc import RPC, RPCException, RPCHandler
from ai_trading_bot.rpc.rpc_types import RPCEntryMsg, RPCExitMsg, RPCOrderMsg, RPCSendMsg
...
class Telegram(RPCHandler):
    ...
    def _add_analyzed_candle(self, pair: str) -> str:
        ...
        if candle_val != "off":
            if candle_val == "ohlc":
                analyzed_df, _ = self._rpc._ai_trading_bot.dataprovider.get_analyzed_dataframe(
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._ai_trading_bot.trading_mode != TradingMode.SPOT:
            force_enter_text += (
    ...
    async def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._ai_trading_bot.strategy.version()
    ...
    async def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._ai_trading_bot.state)
