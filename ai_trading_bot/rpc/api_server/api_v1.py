...
from ai_trading_bot.rpc.api_server.api_pairlists import handleExchangePayload
...
from ai_trading_bot.rpc.api_server.deps import get_config, get_exchange, get_rpc, get_rpc_optional
...
def show_config(rpc: RPC | None = Depends(get_rpc_optional), config=Depends(get_config)):
    ...
    if rpc:
        state = rpc._ai_trading_bot.state
        strategy_version = rpc._ai_trading_bot.strategy.version()
...
def markets(
    ...
    if not rpc or config["runmode"] == RunMode.WEBSERVER:
        ...
        exchange = rpc._ai_trading_bot.exchange
