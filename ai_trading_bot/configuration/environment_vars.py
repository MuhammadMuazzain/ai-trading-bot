...
from ai_trading_bot.constants import ENV_VAR_PREFIX
...
def _flat_vars_to_nested_dict(env_dict: dict[str, Any], prefix: str) -> dict[str, Any]:
    """
    Environment variables must be prefixed with AI_TRADING_BOT.
    AI_TRADING_BOT__{section}__{key}
    :param env_dict: Dictionary to validate - usually os.environ
    :param prefix: Prefix to consider (usually AI_TRADING_BOT__)
...
def enironment_vars_to_dict() -> dict[str, Any]:
    ...
    Relevant variables must follow the AI_TRADING_BOT__{section}__{key} pattern
    :return: Nested dict based on available and relevant variables.
    """
