...
from ai_trading_bot.data.history import get_timerange, load_data
from ai_trading_bot.data.metrics import (
...
from ai_trading_bot.exchange import timeframe_to_prev_date, timeframe_to_seconds
from ai_trading_bot.misc import pair_to_filename
...
from ai_trading_bot.util import get_dry_run_wallet
...
def generate_profit_graph(
    ...
    fig["layout"].update(title="AI Trading Bot Profit plot")
    fig["layout"]["yaxis1"].update(title="Price")
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = "ai_trading_bot-plot-" + pair_s + "-" + timeframe + ".html"
...
def plot_profit(config: Config) -> None:
    ...
    store_plot_file(
        fig,
        filename="ai_trading_bot-profit-plot.html",
        directory=config["user_data_dir"] / "plot",
