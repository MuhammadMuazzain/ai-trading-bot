...
from ai_trading_bot.data.converter import ohlcv_to_dataframe, trades_list_to_df
from ai_trading_bot.edge import PairInfo
from ai_trading_bot.enums import CandleType, MarginMode, RunMode, SignalDirection, TradingMode
from ai_trading_bot.exchange import Exchange, timeframe_to_minutes, timeframe_to_seconds
from ai_trading_bot.trading_bot import TradingBot
from ai_trading_bot.persistence import LocalTrade, Order, Trade, init_db
from ai_trading_bot.resolvers import ExchangeResolver
from ai_trading_bot.util import dt_now, dt_ts
...
EXMS = "ai_trading_bot.exchange.exchange.Exchange"
...
def patched_configuration_load_config_file(mocker, config) -> None:
    mocker.patch(
        "ai_trading_bot.configuration.load_config.load_config_file", lambda *args, **kwargs: config
    )
...
def patch_exchange(
    ...
    # Temporary patch ...
    mocker.patch("ai_trading_bot.exchange.bybit.Bybit.cache_leverage_tiers")
    ...
    if mock_supported_modes:
        mocker.patch(
            f"ai_trading_bot.exchange.{exchange}.{exchange.capitalize()}"
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch("ai_trading_bot.wallets.Wallets.get_free", MagicMock(return_value=free))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch(
        "ai_trading_bot.trading_bot.TradingBot._refresh_active_whitelist",
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch("ai_trading_bot.edge.Edge.calculate", MagicMock(return_value=True))


# Functions for recurrent object patching


def patch_trading_bot(mocker, config) -> None:
    """
    ...
    mocker.patch("ai_trading_bot.trading_bot.RPCManager._init", MagicMock())
    mocker.patch("ai_trading_bot.trading_bot.RPCManager.send_msg", MagicMock())
    patch_whitelist(mocker, config)
    mocker.patch("ai_trading_bot.trading_bot.ExternalMessageConsumer")
    mocker.patch("ai_trading_bot.configuration.config_validation._validate_consumers")


def get_patched_trading_bot(mocker, config) -> TradingBot:
    """
    ...
    :return: TradingBot
    """
    patch_trading_bot(mocker, config)
...
def patch_get_signal(
    ...
    ai_trading_bot.strategy.get_entry_signal = patched_get_entry_signal
    ...
    # returns (enter, exit)
    ai_trading_bot.strategy.get_exit_signal = patched_get_exit_signal

    ai_trading_bot.exchange.refresh_latest_ohlcv = lambda p: None
...
def patch_gc(mocker) -> None:
    mocker.patch("ai_trading_bot.main.gc_set_threshold")
...
def user_dir(mocker, tmp_path) -> Path:
    ...
    mocker.patch("ai_trading_bot.configuration.configuration.create_userdata_dir", return_value=user_dir)
    return user_dir
...
def patch_coingecko(mocker) -> None:
    ...
    mocker.patch.multiple(
        "ai_trading_bot.rpc.fiat_convert.FtCoinGeckoApi",
