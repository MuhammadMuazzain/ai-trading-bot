...
def test_file_dump_json(mocker) -> None:
    file_open = mocker.patch("ai_trading_bot.misc.Path.open", MagicMock())
    json_dump = mocker.patch("rapidjson.dump", MagicMock())
    file_dump_json(Path("somefile"), [1, 2, 3])
    assert file_open.call_count == 1
    assert json_dump.call_count == 1
    file_open = mocker.patch("ai_trading_bot.misc.gzip.open", MagicMock())
    json_dump = mocker.patch("rapidjson.dump", MagicMock())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "sqlite:////ai_trading_bot/user_data/tradesv3.sqlite",
            "sqlite:////ai_trading_bot/user_data/tradesv3.sqlite",
        ),
    ],
)
