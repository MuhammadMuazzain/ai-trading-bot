...
from ai_trading_bot.constants import LAST_BT_RESULT_FN
...
from ai_trading_bot.data.history import load_data, load_pair_history
...
def test_get_latest_backtest_filename(testdatadir, mocker):
    ...
    mocker.patch("ai_trading_bot.data.btanalysis.json_load", return_value={})
...
def test_load_backtest_metadata(mocker, testdatadir):
    ...
    mocker.patch("ai_trading_bot.data.btanalysis.get_backtest_metadata_filename")
    mocker.patch("ai_trading_bot.data.btanalysis.json_load", side_effect=Exception())
    with pytest.raises(
...
def test_load_backtest_data_old_format(testdatadir, mocker):
    ...
    mocker.patch("ai_trading_bot.data.btanalysis.load_backtest_stats", return_value=[])
...
def test_load_trades_from_db(default_conf, fee, is_short, mocker):
    ...
    init_mock = mocker.patch("ai_trading_bot.data.btanalysis.init_db", MagicMock())
...
def test_load_trades(default_conf, mocker):
    db_mock = mocker.patch("ai_trading_bot.data.btanalysis.load_trades_from_db", MagicMock())
    bt_mock = mocker.patch("ai_trading_bot.data.btanalysis.load_backtest_data", MagicMock())
...
def test_calculate_max_drawdown_abs(profits, relative, highd, lowdays, result, result_rel):
    """
    Test case from issue https://github.com/MuhammadMuazzain/ai-trading-bot/issues/6655
