...
from ai_trading_bot.loggers import (
    FTBufferingHandler,
    FtRichHandler,
    setup_logging,
    setup_logging_pre,
)
from ai_trading_bot.loggers.set_log_levels import (
...
def test_reduce_verbosity():
    setup_logging_pre()
    reduce_verbosity_for_bias_tester()
    prior_level = logging.getLogger("ai-trading-bot").getEffectiveLevel()

    assert logging.getLogger("ai_trading_bot.resolvers").getEffectiveLevel() == logging.WARNING
    assert logging.getLogger("ai_trading_bot.strategy.hyper").getEffectiveLevel() == logging.WARNING
    # base level wasn't changed
    assert logging.getLogger("ai-trading-bot").getEffectiveLevel() == prior_level

    restore_verbosity_for_bias_tester()

    assert logging.getLogger("ai_trading_bot.resolvers").getEffectiveLevel() == prior_level
    assert logging.getLogger("ai_trading_bot.strategy.hyper").getEffectiveLevel() == prior_level
    assert logging.getLogger("ai-trading-bot").getEffectiveLevel() == prior_level
    # base level wasn't changed
