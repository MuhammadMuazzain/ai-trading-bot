...
from ai_trading_bot.enums import ExitCheckTuple, ExitType, TradingMode
from ai_trading_bot.persistence import Trade
from ai_trading_bot.persistence.models import Order
from ai_trading_bot.rpc.rpc import RPC
from tests.conftest import EXMS, get_patched_trading_bot, log_has_re, patch_get_signal
...
def test_may_execute_exit_stoploss_on_exchange_multi(default_conf, ticker, fee, mocker) -> None:
    ...
    * 3rd trade is sold via sell-signal
    ...
    mocker.patch("ai_trading_bot.strategy.interface.IStrategy.should_exit", should_sell_mock)
    wallets_mock = mocker.patch("ai_trading_bot.wallets.Wallets.update")
    mocker.patch("ai_trading_bot.wallets.Wallets.get_free", return_value=1000)
    mocker.patch("ai_trading_bot.wallets.Wallets.check_exit_amount", return_value=True)

    ai-trading-bot = get_patched_trading_bot(mocker, default_conf)
    ai_trading_bot.strategy.order_types["stoploss_on_exchange"] = True
    # Switch ordertype to market to close trade immediately
    ai_trading_bot.strategy.order_types["exit"] = "market"
    ...
    assert ai_trading_bot.strategy.confirm_trade_entry.call_count == 3
    ai_trading_bot.strategy.confirm_trade_entry.reset_mock()
    ...
    assert ai_trading_bot.strategy.confirm_trade_entry.call_count == 0
    assert ai_trading_bot.strategy.confirm_trade_exit.call_count == 1
    ai_trading_bot.strategy.confirm_trade_exit.reset_mock()
...
def test_forcebuy_last_unlimited(default_conf, ticker, fee, mocker, balance_ratio, result1) -> None:
    ...
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == 4
    assert ai_trading_bot.wallets.get_trade_stake_amount("XRP/BTC", 5) == result1
    ...
    assert len(trades) == 5
    bals = ai_trading_bot.wallets.get_all_balances()
    ...
    bals2 = ai_trading_bot.wallets.get_all_balances()
    assert bals != bals2
    assert len(bals) == 6
    assert len(bals2) == 5
    assert "LTC" in bals
...
def test_dca_short(default_conf_usdt, ticker_usdt, fee, mocker) -> None:
    ...
    patch_get_signal(ai_trading_bot, enter_long=False, enter_short=True)
    ai_trading_bot.enter_positions()
...
def test_dca_order_adjust(default_conf_usdt, ticker_usdt, leverage, fee, mocker) -> None:
    ...
    patch_get_signal(ai_trading_bot)
    ai_trading_bot.strategy.custom_entry_price = lambda **kwargs: ticker_usdt["ask"] * 0.96
    ai_trading_bot.strategy.leverage = MagicMock(return_value=leverage)
    ai_trading_bot.strategy.minimal_roi = {0: 0.2}
    ...
    ai_trading_bot.strategy.adjust_entry_price = MagicMock(return_value=1.99)
    ai_trading_bot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2
    assert trade.has_open_orders
    ...
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=120)
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=False)
    ...
    ai_trading_bot.strategy.adjust_entry_price = MagicMock(return_value=1.95)
    ai_trading_bot.strategy.adjust_exit_price = MagicMock(side_effect=ValueError)
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=None)
    ai_trading_bot.process()
    trade = Trade.get_trades().first()
    ...
    assert ai_trading_bot.strategy.adjust_exit_price.call_count == 0

    # Fill DCA order
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=None)
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=True)
    ai_trading_bot.strategy.adjust_entry_price = MagicMock(side_effect=ValueError)
    ...
    # Full exit
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=False)
    ai_trading_bot.strategy.custom_exit = MagicMock(return_value="Exit now")
    ai_trading_bot.strategy.adjust_entry_price = MagicMock(return_value=2.02)
    ai_trading_bot.strategy.adjust_exit_price = MagicMock(side_effect=ValueError)
    ai_trading_bot.process()
    trade = Trade.get_trades().first()
    ...
    assert ai_trading_bot.strategy.adjust_entry_price.call_count == 0
    assert ai_trading_bot.strategy.adjust_exit_price.call_count == 1

    ai_trading_bot.strategy.adjust_exit_price = MagicMock(return_value=2.03)
...
def test_dca_order_adjust_entry_replace_fails(
    ...
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=(20, "PeNF"))

    ai_trading_bot.process()

    assert ai_trading_bot.strategy.adjust_trade_position.call_count == 2
    trades = Trade.session.scalars(
        select(Trade)
        .where(Order.ft_is_open.is_(True))
        .where(Order.ft_order_side != "stoploss")
    ...
    ai_trading_bot.strategy.adjust_entry_price = MagicMock(return_value=2.05)
    ...
    assert ai_trading_bot.strategy.adjust_entry_price.call_count == 2
    ...
    entry_mock = mocker.patch(
        "ai_trading_bot.trading_bot.TradingBot.execute_entry", return_value=False
    )
...
def test_dca_exiting(default_conf_usdt, ticker_usdt, fee, mocker, caplog, leverage) -> None:
    ...
    if not spot:
        ...
        default_conf_usdt["margin_mode"] = "isolated"
    ai-trading-bot = get_patched_trading_bot(mocker, default_conf_usdt)
    assert ai_trading_bot.trading_mode == TradingMode.FUTURES if not spot else TradingMode.SPOT
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        get_fee=fee,
    ...
    mocker.patch(f"{EXMS}.get_max_leverage", return_value=10)
    starting_amount = ai_trading_bot.wallets.get_total("USDT")
    ...
    assert trade.open_rate == 2.0
    assert pytest.approx(ai_trading_bot.wallets.get_free("USDT")) == starting_amount - 60
    if spot:
        assert pytest.approx(ai_trading_bot.wallets.get_total("USDT")) == starting_amount - 60
    else:
        assert ai_trading_bot.wallets.get_total("USDT") == starting_amount

    # Too small size
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=-59)
    ai_trading_bot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 1
    assert pytest.approx(trade.stake_amount) == 60
    ...
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=(-20, "PES"))
    ...
    assert pytest.approx(ai_trading_bot.wallets.get_free("USDT")) == expected_profit

    if spot:
        assert pytest.approx(ai_trading_bot.wallets.get_total("USDT")) == expected_profit
    else:
        # total won't change in futures mode, only free / used will.
        assert ai_trading_bot.wallets.get_total("USDT") == starting_amount + trade.realized_profit
    caplog.clear()

    # Sell more than what we got (we got ~20 coins left)
    # Doesn't exit, as the amount is too high.
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=-50)
    ai_trading_bot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2

    # Amount too low...
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=-(trade.stake_amount * 0.99))
    ai_trading_bot.process()
    ...
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=-trade.stake_amount)
    ai_trading_bot.process()
    ...
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=-0.01)
    ai_trading_bot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 3
    assert trade.orders[-1].ft_order_side == "sell"
...
def test_dca_handle_similar_open_order(
    ...
    ai_trading_bot.strategy.custom_entry_price = lambda **kwargs: ticker_usdt["ask"] * 0.96
    ai_trading_bot.strategy.leverage = MagicMock(return_value=leverage)
    ai_trading_bot.strategy.custom_exit = MagicMock(return_value=False)
    ai_trading_bot.strategy.minimal_roi = {0: 0.2}
    ...
    assert ai_trading_bot.strategy.custom_exit.call_count == 0
    ...
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(
        return_value=21
    ...
    ai_trading_bot.strategy.custom_exit.reset_mock()

    # Should Create a new exit order
    ai_trading_bot.exchange.amount_to_contract_precision = MagicMock(return_value=2)
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=-2)
    ...
    ai_trading_bot.exchange.amount_to_contract_precision = MagicMock(return_value=3)
    ai_trading_bot.strategy.adjust_trade_position = MagicMock(return_value=-3)
    ai_trading_bot.process()
    trade = Trade.get_trades().first()
    ...
    ai_trading_bot.strategy.custom_exit_price = MagicMock(return_value=1.95)
    ai_trading_bot.process()
    # Even with open order, trying to exit...
    assert ai_trading_bot.strategy.custom_exit.call_count == 1
    ai_trading_bot.strategy.custom_exit.reset_mock()
