...
from ai_trading_bot.enums import ExitCheckTuple, ExitType, RPCMessageType
from ai_trading_bot.exceptions import ExchangeError, InsufficientFundsError, InvalidOrderException
from ai_trading_bot.trading_bot import TradingBot
from ai_trading_bot.persistence import Order, Trade
...
from tests.trading_bot.test_trading_bot import patch_RPCManager
...
def test_add_stoploss_on_exchange(mocker, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    ai_trading_bot.exit_positions(trades)
...
def test_handle_stoploss_on_exchange(
    ...
    assert ai_trading_bot.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    ...
    assert ai_trading_bot.handle_stoploss_on_exchange(trade) is True
    ...
    trade.is_open = True
    ai_trading_bot.handle_stoploss_on_exchange(trade)
...
def test_handle_stoploss_on_exchange_emergency(
    ...
    ai_trading_bot.config["trailing_stop"] = True
...
def test_handle_stoploss_on_exchange_partial_cancel_here(
    ...
    # AI Trading Bot cancels the stop - but cancel returns a partial filled order.
    stoploss_order_hit = MagicMock(
        return_value={
            "id": "101",
            "status": "open",
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    ai_trading_bot.create_stoploss_order(trade, 200)
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    ai_trading_bot.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    ai_trading_bot.strategy.order_types["stoploss_on_exchange_interval"] = 60
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair="ETH/USDT",
        order_types=ai_trading_bot.strategy.order_types,
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    ai_trading_bot.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    ai_trading_bot.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    ai_trading_bot.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
    ...
    ai_trading_bot.strategy.order_types["stoploss_on_exchange_interval"] = 0
    cancel_order_mock.assert_not_called()
    stoploss_order_mock.assert_not_called()
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee, limit_order) -> None:
    ...
    # setting stoploss
    ai_trading_bot.strategy.stoploss = -0.02
    ...
    ai_trading_bot.active_pair_whitelist = ai_trading_bot.edge.adjust(ai_trading_bot.active_pair_whitelist)
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    ai-trading-bot = get_patched_trading_bot(mocker, default_conf_usdt)
    mocker.patch(f"{EXMS}.cancel_stoploss_order", side_effect=InvalidOrderException())
    mocker.patch("ai_trading_bot.wallets.Wallets.get_free", MagicMock(return_value=300))
    create_order_mock = MagicMock(
        side_effect=[
            {"id": "12345554"},
            {"id": "12345555"},
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    ai-trading-bot = TradingBot(default_conf_usdt)
    ai_trading_bot.strategy.order_types["stoploss_on_exchange"] = True
    patch_get_signal(ai_trading_bot, enter_long=not is_short, enter_short=is_short)
