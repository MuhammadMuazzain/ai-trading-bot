...
def test_worker_state(mocker, default_conf, markets) -> None:
    ...
    assert worker.ai_trading_bot.state is State.STOPPED


def test_worker_running(mocker, default_conf, caplog) -> None:
    mock_throttle = MagicMock()
    mocker.patch("ai_trading_bot.worker.Worker._throttle", mock_throttle)
    mocker.patch("ai_trading_bot.persistence.Trade.stoploss_reinitialization", MagicMock())
    ...
    assert worker.ai_trading_bot.strategy
    assert worker.ai_trading_bot.strategy.dp
    assert isinstance(worker.ai_trading_bot.strategy.dp, DataProvider)
...
def test_worker_paused(mocker, default_conf, caplog) -> None:
    ...
    worker.ai_trading_bot.state = State.PAUSED
...
def test_worker_stopped(mocker, default_conf, caplog) -> None:
    ...
    worker = get_patched_worker(mocker, default_conf)
    worker.ai_trading_bot.state = State.STOPPED
    state = worker._worker(old_state=State.RUNNING)
...
def test_worker_lifecycle(
    ...
    mocker.patch("ai_trading_bot.worker.Worker._throttle", mock_throttle)
    mocker.patch("ai_trading_bot.persistence.Trade.stoploss_reinitialization")
    startup = mocker.patch("ai_trading_bot.trading_bot.TradingBot.startup")

    worker = get_patched_worker(mocker, default_conf)
    worker.ai_trading_bot.state = target_state
...
def test_throttle_sleep_time(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    worker = get_patched_worker(mocker, default_conf)
    sleep_mock = mocker.patch("ai_trading_bot.worker.Worker._sleep")
...
def test_worker_heartbeat_running(default_conf, mocker, caplog):
    ...
    worker.ai_trading_bot.state = State.RUNNING
