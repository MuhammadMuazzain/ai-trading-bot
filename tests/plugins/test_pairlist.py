...
from ai_trading_bot.constants import AVAILABLE_PAIRLISTS
from ai_trading_bot.data.dataprovider import DataProvider
from ai_trading_bot.enums import CandleType, RunMode
from ai_trading_bot.exceptions import OperationalException
from ai_trading_bot.persistence import LocalTrade, Trade
from ai_trading_bot.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist, expand_pairlist
from ai_trading_bot.plugins.pairlistmanager import PairListManager
from ai_trading_bot.resolvers import PairListResolver
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    ai-trading-bot = get_patched_trading_bot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = ai_trading_bot.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(ai_trading_bot.exchange, default_conf, MagicMock())
    with pytest.raises(
        ...
        PairListResolver.load_pairlist(
            "NonexistingPairList", ai_trading_bot.exchange, plm, default_conf, {}, 1
        )
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    ai_trading_bot.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ["ETH/BTC", "TKN/BTC"]
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(ai_trading_bot.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (
        static_pl_conf["exchange"]["pair_whitelist"]
        == ai_trading_bot.config["exchange"]["pair_whitelist"]
    )
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(ai_trading_bot.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == ai_trading_bot.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(ai_trading_bot.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == ai_trading_bot.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = ai_trading_bot.pairlists.verify_blacklist(
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    ai-trading-bot = get_patched_trading_bot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_trading_bot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
    ...
    ai_trading_bot.pairlists.refresh_pairlist()
    assert whitelist == ai_trading_bot.pairlists.whitelist
    ...
    with pytest.raises(
        ...
        PairListManager(ai_trading_bot.exchange, whitelist_conf, MagicMock())
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        "ai_trading_bot.plugins.pairlistmanager.PairListManager",
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    ai-trading-bot = get_patched_trading_bot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_trading_bot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_whitelist_gen(
    ...
    mocker.patch.multiple(
        "ai_trading_bot.persistence.Trade", get_overall_performance=MagicMock(return_value=[])
    )
    ...
    if whitelist_result == "filter_at_the_beginning":
        ...
        ai_trading_bot.pairlists.refresh_pairlist()
        whitelist = ai_trading_bot.pairlists.whitelist
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(
        ...
        get_patched_trading_bot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert ai_trading_bot.pairlists.name_list == [pairlist]
    assert pairlist in str(ai_trading_bot.pairlists.short_desc())
    assert isinstance(ai_trading_bot.pairlists.whitelist, list)
    assert isinstance(ai_trading_bot.pairlists.blacklist, list)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ETH/BTC", "TKN/BTC", "XLTCUSDT"], "is not tradable with AI Trading Bot"),
    ],
)
def test__whitelist_for_active_markets(
    ...
    # Assign starting whitelist
    pairlist_handler = ai_trading_bot.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_invalid_sortvalue(mocker, whitelist_conf):
    ...
    with pytest.raises(OperationalException, match=r"key asdf not in .*"):
        get_patched_trading_bot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    ai-trading-bot = get_patched_trading_bot(mocker, whitelist_conf)
    assert len(ai_trading_bot.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    ai_trading_bot.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(ai_trading_bot.pairlists._pairlist_handlers[0]._pair_cache) == 1
    ai_trading_bot.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        ai-trading-bot = get_patched_trading_bot(mocker, whitelist_conf_agefilter)
        assert ai_trading_bot.exchange.refresh_latest_ohlcv.call_count == 0
        ai_trading_bot.pairlists.refresh_pairlist()
        assert len(ai_trading_bot.pairlists.whitelist) == 3
        assert ai_trading_bot.exchange.refresh_latest_ohlcv.call_count > 0

        ai_trading_bot.pairlists.refresh_pairlist()
        assert len(ai_trading_bot.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert ai_trading_bot.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        ai_trading_bot.pairlists.refresh_pairlist()
        assert len(ai_trading_bot.pairlists.whitelist) == 3
        assert ai_trading_bot.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch(f"{EXMS}.refresh_latest_ohlcv", return_value=ohlcv_data)
        ai_trading_bot.pairlists.refresh_pairlist()
        assert len(ai_trading_bot.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(
    ...
    assert ai_trading_bot.exchange.refresh_latest_ohlcv.call_count == 0
    ai_trading_bot.pairlists.refresh_pairlist()
    assert len(ai_trading_bot.pairlists.whitelist) == expected_length
    assert ai_trading_bot.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = ai_trading_bot.exchange.refresh_latest_ohlcv.call_count
    ai_trading_bot.pairlists.refresh_pairlist()
    assert len(ai_trading_bot.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert ai_trading_bot.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_pricefilter_desc(
    ...
    if desc_expected is not None:
        ai-trading-bot = get_patched_trading_bot(mocker, whitelist_conf)
        short_desc = str(ai_trading_bot.pairlists.short_desc())
...
def test_performance_filter(
    ...
    ai-trading-bot = get_patched_trading_bot(mocker, allowlist_conf)
    ...
    mocker.patch.multiple(
        "ai_trading_bot.persistence.Trade",
        get_overall_performance=MagicMock(return_value=overall_performance),
    )
    ai_trading_bot.pairlists.refresh_pairlist()
    allowlist = ai_trading_bot.pairlists.whitelist
...
def test_MarketCapPairList_filter(
    ...
    mocker.patch(
        "ai_trading_bot.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_categories_list",
        return_value=[
            {"category_id": "layer-1"},
    ...
    gcm_mock = mocker.patch(
        "ai_trading_bot.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_markets",
