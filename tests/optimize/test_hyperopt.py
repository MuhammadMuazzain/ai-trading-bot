...
from ai_trading_bot.commands.optimize_commands import setup_optimize_configuration, start_hyperopt
from ai_trading_bot.data.history import load_data
from ai_trading_bot.enums import ExitType, RunMode
from ai_trading_bot.exceptions import OperationalException
from ai_trading_bot.optimize.hyperopt import Hyperopt
from ai_trading_bot.optimize.hyperopt.hyperopt_auto import HyperOptAuto
from ai_trading_bot.optimize.hyperopt_tools import HyperoptTools
from ai_trading_bot.optimize.optimize_reports import generate_strategy_stats
from ai_trading_bot.optimize.space import SKDecimal
from ai_trading_bot.strategy import IntParameter
...
def test_start_not_installed(mocker, default_conf, import_fails) -> None:
    ...
    mocker.patch("ai_trading_bot.optimize.hyperopt.Hyperopt.start", start_mock)
...
def test_start_no_data(mocker, hyperopt_conf, tmp_path) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch("ai_trading_bot.data.history.load_pair_history", MagicMock(return_value=pd.DataFrame))
    mocker.patch(
        "ai_trading_bot.optimize.hyperopt.hyperopt_optimizer.get_timerange",
...
def test_start_filelock(mocker, hyperopt_conf, caplog) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch("ai_trading_bot.optimize.hyperopt.Hyperopt.__init__", hyperopt_mock)
    patch_exchange(mocker)
    ...
    pargs = get_args(args)
    start_hyperopt(pargs)
    assert log_has("Another running instance of ai-trading-bot Hyperopt detected.", caplog)
...
def test_start_calls_optimizer(mocker, hyperopt_conf, capsys) -> None:
    dumper = mocker.patch("ai_trading_bot.optimize.hyperopt.hyperopt_optimizer.dump")
    dumper2 = mocker.patch("ai_trading_bot.optimize.hyperopt.Hyperopt._save_result")
    mocker.patch(
        "ai_trading_bot.optimize.hyperopt.hyperopt_optimizer.calculate_market_change", return_value=1.5
    )
    mocker.patch("ai_trading_bot.optimize.hyperopt.hyperopt.file_dump_json")

    mocker.patch(
        "ai_trading_bot.optimize.backtesting.Backtesting.load_bt_data",
    ...
    mocker.patch("ai_trading_bot.optimize.hyperopt.hyperopt.INITIAL_POINTS", 2)

    parallel = mocker.patch(
        "ai_trading_bot.optimize.hyperopt.Hyperopt.run_optimizer_parallel",
        MagicMock(
            return_value=[
                {
                    "loss": 1,
...
def test_generate_optimizer(mocker, hyperopt_conf) -> None:
    ...
    mocker.patch(
        "ai_trading_bot.optimize.hyperopt.hyperopt_optimizer.Backtesting.backtest",
    ...
    mocker.patch("ai_trading_bot.configuration.config_validation.validate_config_schema")
    mocker.patch(
        "ai_trading_bot.optimize.hyperopt.hyperopt_optimizer.load", return_value={"XRP/BTC": None}
    )
...
def test_clean_hyperopt(mocker, hyperopt_conf, caplog):
    ...
    mocker.patch(
        "ai_trading_bot.strategy.hyper.HyperStrategyMixin.load_params_from_file",
        MagicMock(return_value={}),
    )
    mocker.patch("ai_trading_bot.optimize.hyperopt.hyperopt.Path.is_file", MagicMock(return_value=True))
    unlinkmock = mocker.patch("ai_trading_bot.optimize.hyperopt.hyperopt.Path.unlink", MagicMock())
...
def test_simplified_interface_all_failed(mocker, hyperopt_conf, caplog) -> None:
    mocker.patch("ai_trading_bot.optimize.hyperopt.hyperopt_optimizer.dump", MagicMock())
    mocker.patch("ai_trading_bot.optimize.hyperopt.hyperopt.file_dump_json")
    mocker.patch(
        "ai_trading_bot.optimize.backtesting.Backtesting.load_bt_data",
        MagicMock(return_value=(MagicMock(), None)),
    ...
    mocker.patch(
        "ai_trading_bot.optimize.hyperopt.hyperopt_auto.HyperOptAuto._generate_indicator_space",
        return_value=[],
    )
...
def test_in_strategy_auto_hyperopt_per_epoch(mocker, hyperopt_conf, tmp_path, fee) -> None:
    ...
    go = mocker.patch(
        "ai_trading_bot.optimize.hyperopt.hyperopt_optimizer.HyperOptimizer.generate_optimizer",
        return_value={
            "loss": 0.05,
            "results_explanation": "foo result",
            "params": {},
...
def test_stake_amount_unlimited_max_open_trades(mocker, hyperopt_conf, tmp_path, fee) -> None:
    ...
    hyperopt = Hyperopt(hyperopt_conf)
    mocker.patch(
        "ai_trading_bot.optimize.hyperopt.hyperopt_optimizer.HyperOptimizer._get_params_dict",
