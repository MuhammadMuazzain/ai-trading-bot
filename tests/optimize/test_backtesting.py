...
from ai_trading_bot import constants
from ai_trading_bot.commands.optimize_commands import setup_optimize_configuration, start_backtesting
from ai_trading_bot.configuration import TimeRange
from ai_trading_bot.data import history
from ai_trading_bot.data.btanalysis import BT_DATA_COLUMNS, evaluate_result_multi
from ai_trading_bot.data.converter import clean_ohlcv_dataframe, ohlcv_fill_up_missing_data
from ai_trading_bot.data.dataprovider import DataProvider
from ai_trading_bot.data.history import get_timerange
from ai_trading_bot.enums import CandleType, ExitType, RunMode
from ai_trading_bot.exceptions import DependencyException, OperationalException
from ai_trading_bot.exchange import timeframe_to_next_date, timeframe_to_prev_date
from ai_trading_bot.optimize.backtest_caching import get_backtest_metadata_filename, get_strategy_run_id
from ai_trading_bot.optimize.backtesting import Backtesting
from ai_trading_bot.persistence import LocalTrade, Trade
from ai_trading_bot.resolvers import StrategyResolver
from ai_trading_bot.util.datetime_helpers import dt_utc
...
def test_setup_bt_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("ai_trading_bot.configuration.configuration.create_datadir", lambda c, x: x)
...
def test_start(mocker, fee, default_conf, caplog) -> None:
    ...
    patch_exchange(mocker)
    mocker.patch("ai_trading_bot.optimize.backtesting.Backtesting.start", start_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    ...
    assert log_has("Starting ai-trading-bot in Backtesting mode", caplog)
...
def test_backtesting_start(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("ai_trading_bot.data.history.get_timerange", get_timerange)
    patch_exchange(mocker)
    mocker.patch("ai_trading_bot.optimize.backtesting.Backtesting.backtest")
    mocker.patch("ai_trading_bot.optimize.backtesting.generate_backtest_stats")
    mocker.patch("ai_trading_bot.optimize.backtesting.show_backtest_results")
    sbs = mocker.patch("ai_trading_bot.optimize.backtesting.store_backtest_results")
    mocker.patch(
        "ai_trading_bot.plugins.pairlistmanager.PairListManager.whitelist",
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch(
        "ai_trading_bot.data.history.history_utils.load_pair_history",
...
def test_backtesting_no_pair_left(default_conf, mocker) -> None:
    ...
    mocker.patch("ai_trading_bot.optimize.backtesting.Backtesting.backtest")
    mocker.patch(
        "ai_trading_bot.plugins.pairlistmanager.PairListManager.whitelist", PropertyMock(return_value=[])
    )
...
def test_backtesting_pairlist_list(default_conf, mocker, tickers) -> None:
    ...
    mocker.patch("ai_trading_bot.plugins.pairlistmanager.PairListManager.refresh_pairlist")
...
def test_backtest__enter_trade_futures(default_conf_usdt, fee, mocker) -> None:
    ...
    mocker.patch(f"{EXMS}.get_max_pair_stake_amount", return_value=float("inf"))
    mocker.patch(
        "ai_trading_bot.persistence.trade_model.price_to_precision", lambda p, *args, **kwargs: p
    )
    mocker.patch(f"{EXMS}.get_max_leverage", return_value=100)
    mocker.patch("ai_trading_bot.optimize.backtesting.price_to_precision", lambda p, *args: p)
    patch_exchange(mocker)
    ...
    backtesting._set_strategy(backtesting.strategylist[0])
    mocker.patch("ai_trading_bot.optimize.backtesting.Backtesting._run_funding_fees")
    pair = "ETH/USDT:USDT"
    row = [
        pd.Timestamp(year=2020, month=1, day=1, hour=5, minute=0),
        0.1,  # Open
    ...
    # ai_trading_bot_liquidation_price = liq + (abs(open_rate - liq) * liq_buffer * side_1)
    ...
    mocker.patch(
        "ai_trading_bot.wallets.Wallets.get_trade_stake_amount", side_effect=DependencyException
    )
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch("ai_trading_bot.optimize.backtesting.Backtesting.backtest", backtestmock)
    ...
    mocker.patch.multiple(
        "ai_trading_bot.optimize.optimize_reports.bt_output",
        text_table_bt_results=text_table_mock,
        text_table_strategy=strattable_mock,
    )
    mocker.patch.multiple(
        "ai_trading_bot.optimize.optimize_reports.optimize_reports",
...
def test_backtest_start_multi_strat_caching(
    ...
    mocker.patch("ai_trading_bot.optimize.backtesting.Backtesting.backtest", backtestmock)
    mocker.patch("ai_trading_bot.optimize.backtesting.show_backtest_results", MagicMock())
    ...
    mocker.patch.multiple(
        "ai_trading_bot.data.btanalysis",
        load_backtest_metadata=load_backtest_metadata,
        load_backtest_stats=load_backtest_stats,
    )
    mocker.patch("ai_trading_bot.optimize.backtesting.get_strategy_run_id", side_effect=["1", "2", "2"])
