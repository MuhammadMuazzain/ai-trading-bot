...
from ai_trading_bot.enums import SignalDirection, State, TradingMode
from ai_trading_bot.exceptions import ExchangeError, InvalidOrderException, TemporaryError
...
from ai_trading_bot.rpc import RPC, RPCException
...
def test_rpc_trade_status(default_conf, ticker, fee, mocker) -> None:
    ...
    # Fill open order ...
    trading_bot.manage_open_orders()
    trades = Trade.get_open_trades()
    trading_bot.exit_positions(trades)
...
def test__rpc_timeunit_profit(
    ...
    trading_bot = get_patched_trading_bot(mocker, default_conf_usdt)
...
def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog, is_short):
    ...
    trading_bot = get_patched_trading_bot(mocker, default_conf)
    trading_bot.strategy.order_types["stoploss_on_exchange"] = True
    create_mock_trades(fee, is_short)
    rpc = RPC(trading_bot)
...
def test_rpc_balance_handle(default_conf_usdt, mocker, tickers, proxy_coin, margin_mode):
    ...
    mocker.patch("ai_trading_bot.rpc.rpc.CryptoToFiatConverter._find_price", return_value=1.2)
    mocker.patch("ai_trading_bot.rpc.telegram.Telegram", MagicMock())
    mocker.patch.multiple(
        EXMS,
...
def test_rpc_pause(mocker, default_conf) -> None:
    ...
    patch_get_signal(trading_bot)
    rpc = RPC(trading_bot)
    trading_bot.state = State.PAUSED
...
def test_rpc_force_exit(default_conf, ticker, fee, mocker) -> None:
    ...
    trading_bot.config["max_open_trades"] = 3
    trading_bot.enter_positions()
...
def test_rpc_force_entry(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    trading_bot.config["max_open_trades"] = 5
    ...
    trading_bot.strategy.position_adjustment_enable = True
    ...
    pair = "XRP/BTC"
    trading_bot = get_patched_trading_bot(mocker, default_conf)
    trading_bot.config["stake_amount"] = 0
    patch_get_signal(trading_bot)
    rpc = RPC(trading_bot)
    pair = "TKN/BTC"
...
def test_rpc_edge_enabled(mocker, edge_conf) -> None:
    ...
    trading_bot = get_patched_trading_bot(mocker, edge_conf)
