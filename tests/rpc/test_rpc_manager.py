...
from ai_trading_bot.enums import RPCMessageType
from ai_trading_bot.rpc import RPCManager
from ai_trading_bot.rpc.api_server.webserver import ApiServer
from tests.conftest import get_patched_trading_bot, log_has


def test__init__(mocker, default_conf) -> None:
    default_conf["telegram"]["enabled"] = False

    rpc_manager = RPCManager(get_patched_trading_bot(mocker, default_conf))
...
def test_cleanup_telegram_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    telegram_mock = mocker.patch("ai_trading_bot.rpc.telegram.Telegram.cleanup", MagicMock())
    default_conf["telegram"]["enabled"] = False

    trading_bot = get_patched_trading_bot(mocker, default_conf)
    rpc_manager = RPCManager(trading_bot)
    rpc_manager.cleanup()
...
def test_send_msg_telegram_disabled(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch("ai_trading_bot.rpc.telegram.Telegram.send_msg", MagicMock())
    default_conf["telegram"]["enabled"] = False
...
def test_send_msg_telegram_error(mocker, default_conf, caplog) -> None:
    mocker.patch("ai_trading_bot.rpc.telegram.Telegram._init", MagicMock())
    mocker.patch("ai_trading_bot.rpc.telegram.Telegram.send_msg", side_effect=ValueError())
    default_conf["telegram"]["enabled"] = True
    trading_bot = get_patched_trading_bot(mocker, default_conf)
    rpc_manager = RPCManager(trading_bot)
    rpc_manager.send_msg({"type": RPCMessageType.STATUS, "status": "test"})
...
def test_process_msg_queue(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch("ai_trading_bot.rpc.telegram.Telegram.send_msg")
    default_conf["telegram"]["enabled"] = True
...
def test_send_msg_webhook_CustomMessagetype(mocker, default_conf, caplog) -> None:
    ...
    default_conf["webhook"] = {"enabled": True, "url": "https://DEADBEEF.com"}
    mocker.patch(
        "ai_trading_bot.rpc.webhook.Webhook.send_msg", MagicMock(side_effect=NotImplementedError)
    )
    rpc_manager = RPCManager(get_patched_trading_bot(mocker, default_conf))
...
def test_startupmessages_telegram_enabled(mocker, default_conf) -> None:
    ...
    trading_bot = get_patched_trading_bot(mocker, default_conf)
    rpc_manager = RPCManager(trading_bot)
    rpc_manager.startup_messages(default_conf, trading_bot.pairlists, trading_bot.protections)
...
def test_init_apiserver_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    run_mock = MagicMock()
    mocker.patch("ai_trading_bot.rpc.api_server.ApiServer.start_api", run_mock)
    default_conf["telegram"]["enabled"] = False
    rpc_manager = RPCManager(get_patched_trading_bot(mocker, default_conf))
