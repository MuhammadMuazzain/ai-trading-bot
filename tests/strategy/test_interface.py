...
from ai_trading_bot.constants import CUSTOM_TAG_MAX_LENGTH
from ai_trading_bot.data.dataprovider import DataProvider
from ai_trading_bot.data.history import load_data
from ai_trading_bot.enums import ExitCheckTuple, ExitType, HyperoptState, SignalDirection
from ai_trading_bot.exceptions import OperationalException, StrategyError
from ai_trading_bot.optimize.hyperopt_tools import HyperoptStateContainer
...
from ai_trading_bot.strategy.hyper import detect_parameters
...
def test_advise_all_indicators_copy(mocker, default_conf, testdatadir) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    aimock = mocker.patch("ai_trading_bot.strategy.interface.IStrategy.advise_indicators")
    timerange = TimeRange.parse_timerange("1510694220-1510700340")
    data = load_data(testdatadir, "1m", ["UNITTEST/BTC"], timerange=timerange, fill_up_missing=True)
...
def test_auto_hyperopt_interface_loadparams(default_conf, mocker, caplog):
    ...
    mocker.patch("ai_trading_bot.strategy.hyper.HyperoptTools.load_params", return_value=expected_result)
    PairLocks.timeframe = default_conf["timeframe"]
    strategy = StrategyResolver.load_strategy(default_conf)
    ...
    mocker.patch(
        "ai_trading_bot.strategy.hyper.HyperoptTools.load_params", MagicMock(side_effect=ValueError())
    )
