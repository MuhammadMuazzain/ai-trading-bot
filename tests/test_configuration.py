...
from ai_trading_bot.configuration import Configuration, validate_config_consistency
from ai_trading_bot.configuration.config_secrets import sanitize_config
from ai_trading_bot.configuration.config_validation import validate_config_schema
from ai_trading_bot.configuration.deprecated_settings import (
    check_conflicting_settings,
    process_deprecated_setting,
    process_removed_setting,
    process_temporary_deprecated_settings,
)
from ai_trading_bot.configuration.environment_vars import _flat_vars_to_nested_dict
from ai_trading_bot.configuration.load_config import (
    load_config_file,
    load_file,
    load_from_files,
    log_config_error_range,
)
from ai_trading_bot.constants import DEFAULT_DB_DRYRUN_URL, DEFAULT_DB_PROD_URL, ENV_VAR_PREFIX
from ai_trading_bot.enums import RunMode
from ai_trading_bot.exceptions import ConfigurationError, OperationalException
...
def test_load_config_file(default_conf, mocker, caplog) -> None:
    ...
    file_mock = mocker.patch(
        "ai_trading_bot.configuration.load_config.Path.open",
...
def test_load_config_file_error(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch(
        "ai_trading_bot.configuration.load_config.Path.open", mocker.mock_open(read_data=filedata)
    )
...
def test_load_config_combine_dicts(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("ai_trading_bot.configuration.load_config.load_config_file", configsmock)
...
def test_print_config(default_conf, mocker, caplog) -> None:
    ...
    configsmock = MagicMock(side_effect=config_files)
    mocker.patch("ai_trading_bot.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch("ai_trading_bot.configuration.configuration.load_from_files", configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        "ai_trading_bot.configuration.configuration.Path.open",
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog, tmp_path) -> None:
    ...
    mocker.patch("ai_trading_bot.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "ai_trading_bot.configuration.configuration.create_userdata_dir",
    ...
    assert log_has(f"Using user-data directory: {tmp_path / 'ai-trading-bot'} ...", caplog)
    assert "user_data_dir" in config
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("ai_trading_bot.loggers.logging.config.dictConfig", MagicMock)
    arglist = ["trade", "-vvv"]
    args = Arguments(arglist).get_parsed_arg()
...
def test_pairlist_resolving_fallback(mocker, tmp_path):
    ...
    mocker.patch(
        "ai_trading_bot.configuration.configuration.load_file",
...
def test_flat_vars_to_nested_dict(caplog):
    test_args = {
        "AI_TRADING_BOT__EXCHANGE__SOME_SETTING": "true",
        "AI_TRADING_BOT__EXCHANGE__SOME_FALSE_SETTING": "false",
        "AI_TRADING_BOT__EXCHANGE__CONFIG__whatever": "sometime",
        "AI_TRADING_BOT__EXIT_PRICING__PRICE_SIDE": "bid",
        "AI_TRADING_BOT__EXIT_PRICING__cccc": "500",
        "AI_TRADING_BOT__STAKE_AMOUNT": "200.05",
        "AI_TRADING_BOT__TELEGRAM__CHAT_ID": "2151",
        "NOT_RELEVANT": "200.0",  # Will be ignored
        "AI_TRADING_BOT__ARRAY": '[{"name":"default","host":"xxx"}]',
        "AI_TRADING_BOT__EXCHANGE__PAIR_WHITELIST": '["BTC/USDT", "ETH/USDT"]',
        # Fails due to trailing comma
        "AI_TRADING_BOT__ARRAY_TRAIL_COMMA": '[{"name":"default","host":"xxx",}]',
        # Object fails
        "AI_TRADING_BOT__OBJECT": '{"name":"default","host":"xxx"}',
    }
    expected = {
        "stake_amount": 200.05,
        "exit_pricing": {
    ...
    assert log_has("Loading variable 'AI_TRADING_BOT__EXCHANGE__SOME_SETTING'", caplog)
