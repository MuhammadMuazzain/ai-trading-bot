...
def extract_config_json_schema():
    try:
        ...
        from ai_trading_bot.config_schema import CONF_SCHEMA
    except ImportError:
        # If ai-trading-bot is not installed, add the parent directory to sys.path
        # to import directly from the source
        import sys

        script_dir = Path(__file__).parent
        ai_trading_bot_dir = script_dir.parent
        sys.path.insert(0, str(ai_trading_bot_dir))
