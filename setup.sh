...
function updateenv() {
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing AI Trading Bot"
        exit 1
    fi
...
function config() {
    echo_block "Please use 'ai-trading-bot new-config -c user_data/config.json' to generate a new configuration file."
}
...
function install() {
    ...
    echo "You can now use the bot by executing 'source .venv/bin/activate; ai-trading-bot <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .venv/bin/activate; ai-trading-bot --help'."
    echo "You verify that ai-trading-bot is installed successfully by running 'source .venv/bin/activate; ai-trading-bot --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install ai-trading-bot from scratch"
