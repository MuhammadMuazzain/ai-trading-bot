...
function Main {
  ...
  # Install ai-trading-bot from setup using the virtual environment's Python
  Write-Log "Installing ai-trading-bot from setup..."
  pip install -e . 2>&1 | Out-File $LogFilePath -Append
  if ($LASTEXITCODE -ne 0) {
    Write-Log "Failed to install ai_trading_bot." -Level 'ERROR'
    Exit-Script -exitCode 1
  }

  Write-Log "Installing freqUI..."
  python ai-trading-bot install-ui 2>&1 | Out-File $LogFilePath -Append
  if ($LASTEXITCODE -ne 0) {
