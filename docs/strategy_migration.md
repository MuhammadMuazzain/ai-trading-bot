...
We have put a great effort into keeping compatibility with existing strategies, so if you just want to continue using ai-trading-bot in __spot markets__, there should be no changes necessary for now.
...
class MyCoolFreqaiModel(BaseRegressionModel):
    """
    Some cool custom IFreqaiModel you made before AI Trading Bot version 2023.6
    """
    def train(
