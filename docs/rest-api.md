...
We advise consuming the API by using the supported `ai-trading-bot-client` package (also available as `scripts/rest_client.py`).

This command can be installed independent of any running ai-trading-bot bot by using `pip install ai-trading-bot-client`.

This module is designed to be lightweight, and only depends on the `requests` and `python-rapidjson` modules, skipping all heavy dependencies ai-trading-bot otherwise needs.

``` bash
ai-trading-bot-client <command> [optional parameters]
```
...
``` bash
ai-trading-bot-client --config rest_config.json <command> [optional parameters]
```

Commands with many arguments may require keyword arguments (for clarity) - which can be provided as follows:

``` bash
ai-trading-bot-client --config rest_config.json forceenter BTC/USDT long enter_tag=GutFeeling
```

This method will work for all arguments - check the "show" command for a list of available parameters.

??? Note "Programmatic use"
    The `ai-trading-bot-client` package (installable independent of ai_trading_bot) can be used in your own scripts to interact with the ai-trading-bot API.
    to do so, please use the following:

    ``` python
    from ai_trading_bot_client import RestApiClient
    

    client = RestApiClient(server_url, username, password)
...
The API Server includes a websocket endpoint for subscribing to RPC messages from the ai-trading-bot Bot.
...
This is also used to setup [Producer/Consumer mode](producer-consumer.md) in AI Trading Bot.
...
For a list of message types, please refer to the RPCMessageType enum in `ai_trading_bot/enums/rpcmessagetype.py`
...
Please make sure to replace `<ai_trading_bot_listen_ip>` (and the subsequent port) with the IP and Port matching your configuration/setup.
...
http {
    ...
    server {
        ...
        location / {
            ...
            proxy_pass http://<ai_trading_bot_listen_ip>:8080;
...
!!! Tip "SSL certificates"
    ...
    While this will protect your data in transit, we do not recommend to run the ai-trading-bot API outside of your private network (VPN, SSH tunnel).
...
!!! Note
    The below should be done in an application (a AI Trading Bot REST API client, which fetches info via API), and is not intended to be used on a regular basis.

AI Trading Bot's REST API also offers JWT (JSON Web Tokens).
You can login using the following command, and subsequently use the resulting access_token.

``` bash
> curl -X POST --user AI Trading Botr http://localhost:8080/api/v1/token/login
