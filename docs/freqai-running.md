...
```bash
ai-trading-bot trade --strategy FreqaiExampleStrategy --config config_freqai.example.json --freqaimodel LightGBMRegressor
```
...
```bash
ai-trading-bot backtesting --strategy FreqaiExampleStrategy --strategy-path ai_trading_bot/templates --config config_examples/config_freqai.example.json --freqaimodel LightGBMRegressor --timerange 20210501-20210701
```
...
You can hyperopt using the same command as for [typical AI Trading Bot hyperopt](hyperopt.md):

```bash
ai-trading-bot hyperopt --hyperopt-loss SharpeHyperOptLoss --strategy FreqaiExampleStrategy --freqaimodel LightGBMRegressor --strategy-path ai_trading_bot/templates --config config_examples/config_freqai.example.json --timerange 20220428-20220507
```
...
!!! note "Availability"
    FreqAI includes tensorboard for a variety of models, including XGBoost, all PyTorch models, Reinforcement Learning, and Catboost. If you would like to see Tensorboard integrated into another model type, please open an issue on the [AI Trading Bot GitHub](https://github.com/MuhammadMuazzain/ai-trading-bot/issues)
