...
FreqAI is configured through the typical [AI Trading Bot config file](configuration.md) and the standard [AI Trading Bot strategy](strategy-customization.md). Examples of FreqAI config and strategy files can be found in `config_examples/config_freqai.example.json` and `ai_trading_bot/templates/FreqaiExampleStrategy.py`, respectively.
...
The FreqAI strategy requires including the following lines of code in the standard [AI Trading Bot strategy](strategy-customization.md):
...
!!! Note
    ...
    (as exemplified in `ai_trading_bot/templates/FreqaiExampleStrategy.py`).
...
The `startup_candle_count` in the FreqAI strategy needs to be set up in the same way as in the standard AI Trading Bot strategy (see details [here](strategy-customization.md#strategy-startup-period)). This value is used by AI Trading Bot to ensure that a sufficient amount of data is provided when calling the `dataprovider`, to avoid any NaNs at the beginning of the first training. You can easily set this value by identifying the longest period (in candle units) which is passed to the indicator creation functions (e.g., TA-Lib functions). In the presented example, `startup_candle_count` is 20 since this is the maximum value in `indicators_periods_candles`.
...
!!! Note
    ...
    ```
    2022-08-31 15:14:04 - ai_trading_bot.freqai.data_kitchen - INFO - dropped 0 training points due to NaNs in populated dataset 4319.
    ```
...
```bash
ai-trading-bot trade --config config_examples/config_freqai.example.json --strategy FreqaiExampleStrategy --freqaimodel PyTorchMLPRegressor --strategy-path ai_trading_bot/templates 
```
...
!!! Note "Installation/docker"
    ...
    PyTorch dropped support for macOS x64 (intel based Apple devices) in version 2.3. Subsequently, ai-trading-bot also dropped support for PyTorch on this platform.
...
??? Note "Setting Class Names for Classifiers"
    ...
    def set_freqai_targets(self, dataframe: DataFrame, metadata: dict, **kwargs) -> DataFrame:
        ...
        return dataframe
    ```
    To see a full example, you can refer to the [classifier test strategy class](https://github.com/MuhammadMuazzain/ai-trading-bot/blob/develop/tests/strategy/strats/freqai_test_classifier.py).
