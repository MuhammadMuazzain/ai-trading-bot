...
AI Trading Bot supports spot trading, as well as (isolated) futures trading for some selected exchanges. Please refer to the [documentation start page](index.md#supported-futures-exchanges-experimental) for an up-to-date list of supported exchanges.

### Can my bot open short positions?

AI Trading Bot can open short positions in futures markets.
...
In spot markets, you can in some cases use leveraged spot tokens, which reflect an inverted pair (eg. BTCUP/USD, BTCDOWN/USD, ETHBULL/USD, ETHBEAR/USD,...) which can be traded with AI Trading Bot.
...
* When you work with your strategy & hyperopt file you should use a proper code editor like VSCode or PyCharm. A good code editor will provide syntax highlighting as well as line numbers, making it easy to find syntax errors (most likely pointed out by AI Trading Bot during startup).

## AI Trading Bot common questions

### Can ai-trading-bot open multiple positions on the same pair in parallel?

No. AI Trading Bot will only open one position per pair at a time.
...
Running the bot with `ai-trading-bot trade --config config.json` shows the output `ai_trading_bot: command not found`.
...
While ai-trading-bot can handle this (it'll sell 99 COIN), fees are often below the minimum tradable lot-size (you can only trade full COIN, not 0.9 COIN).
Leaving the dust (0.9 COIN) on the exchange makes usually sense, as the next time ai-trading-bot buys COIN, it'll eat into the remaining small balance, this time selling everything it bought, and therefore slowly declining the dust balance (although it most likely will never reach exactly 0).

Where possible (e.g. on binance), the use of the exchange's dedicated fee currency will fix this.
On binance, it's sufficient to have BNB in your account, and have "Pay fees in BNB" enabled in your profile. Your BNB balance will slowly decline (as it's used to pay fees) - but you'll no longer encounter dust (AI Trading Bot will include the fees in the profit calculations).
...
AI Trading Bot will update the exchange balance when necessary (Before placing an order).
...
AI Trading Bot will not provide incomplete candles to strategies. Using incomplete candles will lead to repainting and consequently to strategies with "ghost" buys, which are impossible to both backtest, and verify after they happened.
...
Please look at the [advanced setup documentation Page](advanced-setup.md#running-multiple-instances-of-ai_trading_bot).
...
Usually, you can use `ai-trading-bot list-strategies` to list all available strategies. 
...
Irrespectively of the reason, AI Trading Bot will fill up these candles with "empty" candles, where open, high, low and close are set to the previous candle close - and volume is empty. In a chart, this will look like a `_` - and is aligned with how exchanges usually represent 0 volume candles.
...
As a consequence, AI Trading Bot will not enter a trade for this pair - as trading on old information is usually not what is desired.
...
By default, the bot writes its log into stderr stream. This is implemented this way so that you can easily separate the bot's diagnostics messages from Backtesting, Edge and Hyperopt results, output from other various AI Trading Bot utility sub-commands, as well as from the output of your custom `print()`'s you may have inserted into your strategy. So if you need to search the log messages with the grep utility, you need to redirect stderr to stdout and disregard stdout.

* In unix shells, this normally can be done as simple as:
```shell
$ ai-trading-bot --some-options 2>&1 >/dev/null | grep 'something'
```
...
```shell
$ ai-trading-bot --some-options 2> >(grep 'something') >/dev/null
```
or
```shell
$ ai-trading-bot --some-options 2> >(grep -v 'something' 1>&2)
```

* You can also write the copy of AI Trading Bot log messages to a file with the `--logfile` option:
```shell
$ ai-trading-bot --logfile /path/to/mylogfile.log --some-options
```
and then grep it as:
```shell
...
On Windows, the `--logfile` option is also supported by AI Trading Bot and you can use the `findstr` command to search the log for the string of interest:
```
...
### Why does ai-trading-bot not have GPU support?

First of all, most indicator libraries don't have GPU support - as such, there would be little benefit for indicator calculations.
The GPU improvements would only apply to pandas-native calculations - or ones written by yourself.

For hyperopt, ai-trading-bot is using scikit-optimize, which is built on top of scikit-learn.
...
```bash
ai-trading-bot hyperopt --hyperopt-loss SharpeHyperOptLossDaily --strategy SampleStrategy -e 1000
```

### Why does it take a long time to run hyperopt?

* Discovering a great strategy with Hyperopt takes time. Study github.com/MuhammadMuazzain/ai-trading-bot, the AI Trading Bot documentation page, join the AI Trading Bot [discord community](https://github.com/MuhammadMuazzain/ai-trading-bot/issues). While you patiently wait for the most advanced, free crypto bot in the world, to hand you a possible golden strategy specially designed just for you.
...
Example:
`ai-trading-bot --config config.json --strategy SampleStrategy --hyperopt SampleHyperopt -e 1000 --timerange 20190601-20200601`

## Edge module

### Edge implements interesting approach for controlling position size, is there any theory behind it?

The Edge module is mostly a result of brainstorming of [@mishaker](https://github.com/mishaker) and [@creslinux](https://github.com/creslinux) ai-trading-bot team members.
...
AI Trading Bot is using exclusively the following official channels:

* [project GitHub issues server](https://github.com/MuhammadMuazzain/ai-trading-bot/issues)
* [AI Trading Bot documentation (https://github.com/MuhammadMuazzain/ai-trading-bot)](https://github.com/MuhammadMuazzain/ai-trading-bot)
* [AI Trading Bot github organization](https://github.com/MuhammadMuazzain/ai-trading-bot)

Nobody affiliated with the ai-trading-bot project will ask you about your exchange keys or anything else exposing your funds to exploitation.
...
Failing to follow these guidelines will not be responsibility of ai_trading_bot.

## "AI Trading Bot token"

AI Trading Bot does not have a Crypto token offering.

Token offerings you find on the internet referring AI Trading Bot, FreqAI or freqUI must be considered to be a scam, trying to exploit ai_trading_bot's popularity for their own, nefarious gains.
