...
To download data (candles / OHLCV) needed for backtesting and hyperoptimization use the `ai-trading-bot download-data` command.

If no additional parameter is specified, ai-trading-bot will download data for `"1m"` and `"5m"` timeframes for the last 30 days.
...
!!! Tip "Tip: Updating existing data"
    If you already have backtesting data available in your data-directory and would like to refresh this data up to today, ai-trading-bot will automatically calculate the missing timerange for the existing pairs and the download will occur from the latest available point until "now", neither `--days` or `--timerange` parameters are required. AI Trading Bot will keep the available data and only download the missing data.  
...
!!! Tip "Downloading all data for one quote currency"
    ...
    `ai-trading-bot download-data --exchange binance --pairs ".*/USDT" <...>`. The provided "pairs" string will be expanded to contain all active pairs on the exchange.
...
```bash
ai-trading-bot download-data --exchange binance
```
...
```bash
ai-trading-bot download-data --exchange binance --pairs ETH/USDT XRP/USDT BTC/USDT
```

or as regex (in this case, to download all active USDT pairs)

```bash
ai-trading-bot download-data --exchange binance --pairs ".*/USDT"
```
...
``` bash
ai-trading-bot download-data --exchange binance --pairs ETH/USDT XRP/USDT BTC/USDT --prepend --timerange 20210101-20220101
```

!!! Note
    AI Trading Bot will ignore the end-date in this mode if data is available, updating the end-date to the existing data start point.

### Data format

AI Trading Bot currently supports the following data-formats:
...
``` bash
time ai-trading-bot list-data --show-timerange --data-format-ohlcv <dataformat>
```
...
The following command will convert all candle (OHLCV) data available in `~/.ai_trading_bot/data/binance` from json to jsongz, saving diskspace in the process.
It'll also remove original json data files (`--erase` parameter).

``` bash
ai-trading-bot convert-data --format-from json --format-to jsongz --datadir ~/.ai_trading_bot/data/binance -t 5m 15m --erase
```
...
The following command will convert all available trade-data in `~/.ai_trading_bot/data/kraken` from jsongz to json.
It'll also remove original jsongz data files (`--erase` parameter).

``` bash
ai-trading-bot convert-trade-data --format-from jsongz --format-to json --datadir ~/.ai_trading_bot/data/kraken --erase
```
...
``` bash
ai-trading-bot trades-to-ohlcv --exchange kraken -t 5m 1h 1d --pairs BTC/EUR ETH/EUR
```
...
```bash
> ai-trading-bot list-data --userdir ~/.ai_trading_bot/user_data/
...
``` bash
> ai-trading-bot list-data --show --trades
...
```bash
ai-trading-bot download-data --exchange kraken --pairs XRP/EUR ETH/EUR --days 20 --dl-trades
```
