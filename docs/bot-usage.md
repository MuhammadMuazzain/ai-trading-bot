...
!!! Note
    If you've used `setup.sh`, don't forget to activate your virtual environment (`source .venv/bin/activate`) before running ai-trading-bot commands.
...
```bash
ai-trading-bot trade -c path/far/far/away/config.json
```
...
```bash
ai-trading-bot trade -c ./config.json
```

and specify both configuration files when running in the normal Live Trade Mode:

```bash
ai-trading-bot trade -c ./config.json -c path/to/secrets/keys.config.json
```
...
AI Trading Bot allows the creation of a user-data directory using `ai-trading-bot create-userdir --userdir someDirectory`.
...
```bash
ai-trading-bot trade --strategy AwesomeStrategy --strategy-path /some/directory
```
...
```bash
ai-trading-bot trade -c config.json --db-url sqlite:///tradesv3.dry_run.sqlite
```
