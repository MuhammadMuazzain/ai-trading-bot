...
!!! Bug
    Hyperopt can crash when used with only 1 CPU Core as found out in [Issue #1133](https://github.com/MuhammadMuazzain/ai-trading-bot/issues/1133)
...
!!! Tip "Quickly optimize ROI, stoploss and trailing stoploss"
    ...
    ai-trading-bot hyperopt --hyperopt-loss SharpeHyperOptLossDaily --spaces roi stoploss trailing --strategy MyWorkingStrategy --config config.json -e 100
    ```
...
For every new set of parameters, ai-trading-bot will run first `populate_entry_trend()` followed by `populate_exit_trend()`, and then run the regular backtesting process to simulate trades.
...
By default, we assume a stoploss of 5% - and a take-profit (`minimal_roi`) of 10% - which means ai-trading-bot will sell the trade once 10% profit has been reached.
...
from ai_trading_bot.strategy import (BooleanParameter, CategoricalParameter, DecimalParameter, 
                                IStrategy, IntParameter)
import ai_trading_bot.vendor.qtpylib.indicators as qtpylib
...
AI Trading Bot can also optimize protections. How you optimize protections is up to you, and the following should be considered as example only.
...
`ai-trading-bot hyperopt --hyperopt-loss SharpeHyperOptLossDaily --strategy MyAwesomeStrategy --spaces protection`
...
!!! Note
    ...
    AI Trading Bot will also automatically change the "--enable-protections" flag if the protection space is selected.
...
* `ShortTradeDurHyperOptLoss` - (default legacy AI Trading Bot hyperoptimization loss function) - Mostly for short trade duration and avoiding losses.
...
```bash
ai-trading-bot hyperopt --config config.json --hyperopt-loss <hyperoptlossname> --strategy <strategyname> -e 500 --spaces all
```
...
```bash
ai-trading-bot hyperopt --strategy <strategyname> --timerange 20210101-20210201
```
...
If you are optimizing ROI, AI Trading Bot creates the 'roi' optimization hyperspace for you -- it's the hyperspace of components for the ROI tables. By default, each ROI table generated by the AI Trading Bot consists of 4 rows (steps). Hyperopt implements adaptive ranges for ROI tables with ranges for values in the ROI steps that depend on the timeframe used. By default the values vary in the following ranges (for some of the most used timeframes, values are rounded to 3 digits after the decimal point):
...
If you have the `generate_roi_table()` and `roi_space()` methods in your custom hyperopt, remove them in order to utilize these adaptive ROI tables and the ROI hyperoptimization space generated by AI Trading Bot by default.
...
If you are optimizing stoploss values, AI Trading Bot creates the 'stoploss' optimization hyperspace for you. By default, the stoploss values in that hyperspace vary in the range -0.35...-0.02, which is sufficient in most cases.

If you have the `stoploss_space()` method in your custom hyperopt file, remove it in order to utilize Stoploss hyperoptimization space generated by AI Trading Bot by default.
...
By default, hyperopt emulates the behavior of the AI Trading Bot Live Run/Dry Run, where only one
...
AI Trading Bot tries to counter the "local minima" problem by using new, randomized points in this case.
...
If you want to output debug messages from your strategy, you can use the `logging` module. By default, AI Trading Bot will output all messages with a level of `INFO` or higher. 
...
* You may have added parameters to hyperopt in `populate_indicators()` where they will be calculated only once **for all epochs**. If you are, for example, trying to optimise multiple SMA timeperiod values, the hyperoptable timeperiod parameter should be placed in `populate_entry_trend()` which is calculated every epoch. See [Optimizing an indicator parameter](https://github.com/MuhammadMuazzain/ai-trading-bot/hyperopt/#optimizing-an-indicator-parameter).
