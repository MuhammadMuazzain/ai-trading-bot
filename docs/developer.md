...
This page is intended for developers of AI Trading Bot, people who want to contribute to the AI Trading Bot codebase or documentation, or people who want to understand the source code of the application they're running.

All contributions, bug reports, bug fixes, documentation improvements, enhancements and ideas are welcome. We [track issues](https://github.com/MuhammadMuazzain/ai-trading-bot/issues) on [GitHub](https://github.com) and also have a dev channel on [discord](https://github.com/MuhammadMuazzain/ai-trading-bot/issues) where you can ask questions.

## Documentation

Documentation is available at [https://github.com/MuhammadMuazzain/ai-trading-bot](https://github.com/MuhammadMuazzain/ai-trading-bot/) and needs to be provided with every new feature PR.
...
Before opening a pull request, please familiarize yourself with our [Contributing Guidelines](https://github.com/MuhammadMuazzain/ai-trading-bot/blob/develop/CONTRIBUTING.md).
...
This gives developers the ability to start the bot with all required dependencies *without* needing to install any ai-trading-bot specific dependencies on your local machine.
...
If necessary, the AI Trading Bot team can assist and give guidance with writing good tests (however please don't expect anyone to write the tests for you).
...
AI Trading Bot uses 2 main methods to check log content in tests, `log_has()` and `log_has_re()` (to check using regex, in case of dynamic log-messages).
...
To debug ai_trading_bot, we recommend VSCode (with the Python extension) with the following launch configuration (located in `.vscode/launch.json`).
Details will obviously vary between setups - but this should work to get you started.

``` json
{
    "name": "ai-trading-bot trade",
    "type": "debugpy",
    "request": "launch",
    "module": "ai-trading-bot",
...
A similar setup can also be taken for Pycharm - using `ai-trading-bot` as module name, and setting the command line arguments as "parameters".
...
AI Trading Bot Exceptions all inherit from `AI Trading BotException`.
...
First of all, have a look at the [VolumePairList](https://github.com/MuhammadMuazzain/ai-trading-bot/blob/develop/ai_trading_bot/plugins/pairlist/VolumePairList.py) Handler, and best copy this file with a name of your new Pairlist Handler.
...
!!! Note
    This section is a Work in Progress and is not a complete guide on how to test a new exchange with AI Trading Bot.
...
Also try to use `ai-trading-bot download-data` for an extended timerange (multiple months) and verify that the data downloaded correctly (no holes, the specified timerange was actually downloaded).
...
from ai_trading_bot.data.converter import ohlcv_to_dataframe
...
file = Path('ai_trading_bot/exchange/binance_leverage_tiers.json')
...
``` bash
jupyter nbconvert --ClearOutputPreprocessor.enabled=True --inplace ai_trading_bot/templates/strategy_analysis_example.ipynb
jupyter nbconvert --ClearOutputPreprocessor.enabled=True --to markdown ai_trading_bot/templates/strategy_analysis_example.ipynb --stdout > docs/strategy_analysis_example.md
```
...
* Docker images contain a file, `/MuhammadMuazzain/ai-trading-bot_commit` containing the commit this image is based of.
...
* Edit `ai_trading_bot/__init__.py` and add the version matching the current date (for example `2019.7` for July 2019). Minor versions can be `2019.7.1` should we need to do a second release that month. Version numbers must follow allowed versions from PEP0440 to avoid failures pushing to pypi.
* Commit this part.
