...
Creates the directory structure to hold your files for ai_trading_bot.
...
!!! Warning
    Only vital questions are asked. AI Trading Bot offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)

### Create config examples

```
$ ai-trading-bot new-config --config user_data/config_binance.json
...
```bash
ai-trading-bot new-strategy --userdir ~/.ai_trading_bot/ --strategy AwesomeStrategy
```

Using the advanced template (populates all optional functions and methods)

```bash
ai-trading-bot new-strategy --strategy AwesomeStrategy --template advanced
```
...
``` bash
ai-trading-bot list-strategies
```

Example: Search strategies  directory within the userdir.

``` bash
ai-trading-bot list-strategies --userdir ~/.ai_trading_bot/
```

Example: Search dedicated strategy path.

``` bash
ai-trading-bot list-strategies --strategy-path ~/.ai_trading_bot/strategies/
```
...
```
$ ai-trading-bot list-exchanges
Exchanges available for AI Trading Bot:
...
Example: see all exchanges supported by the ccxt library (including 'bad' ones, i.e. those that are known to not work with AI Trading Bot)

```
$ ai-trading-bot list-exchanges -a
...
```
$ ai-trading-bot list-timeframes -c config_binance.json
...
Timeframes available for the exchange `binance`: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
```

* Example: enumerate exchanges available for AI Trading Bot and print timeframes supported by each of them:
```
$ for i in `ai-trading-bot list-exchanges -1`; do ai-trading-bot list-timeframes --exchange $i; done
```
...
For pairs traded by AI Trading Bot the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
```
$ ai-trading-bot list-pairs --quote USD --print-json
```
...
```
$ ai-trading-bot list-pairs -c config_binance.json --all --base BTC ETH --quote USDT USD --print-list
```

* Print all markets on exchange "Kraken", in the tabular format:

```
$ ai-trading-bot list-markets --exchange kraken --all
```
...
```
ai-trading-bot test-pairlist --config config.json --quote USDT BTC
```

## Convert database

`ai-trading-bot convert-db` can be used to convert your database from one system to another (sqlite -> postgres, postgres -> other postgres), migrating all trades, orders and Pairlocks.
...
!!! Warning
    Please ensure to only use this on an empty target database. AI Trading Bot will perform a regular migration, but may fail if entries already existed.
...
Run ai-trading-bot in webserver mode.
AI Trading Bot will start the webserver and allow FreqUI to start and control backtesting processes.
...
You can use `docker compose run --rm -p 127.0.0.1:8080:8080 ai-trading-bot webserver` to start a one-off container that'll be removed once you stop it. This assumes that port 8080 is still available and no other bot is running on that port.
...
```
ai-trading-bot hyperopt-list --profitable --no-details
```
...
```
ai-trading-bot hyperopt-show -n 168
```

Prints JSON data with details for the last best epoch (i.e., the best of all epochs):

```
ai-trading-bot hyperopt-show --best -n -1 --print-json --no-header
```
...
``` bash
ai-trading-bot show-trades --db-url sqlite:///tradesv3.sqlite --trade-ids 2 3 --print-json
```
