...
## Install ai-trading-bot automatically
...
## Install ai-trading-bot manually
...
As compiling from source on windows has heavy dependencies (requires a partial visual studio installation), AI Trading Bot provides these dependencies (in the binary wheel format) for the latest 3 Python versions (3.10, 3.11 and 3.12) and for 64bit Windows.
These Wheels are also used by CI running on windows, and are therefore tested together with ai_trading_bot.

Other versions must be downloaded from the above link.

``` powershell
cd \path\ai_trading_bot
python -m venv .venv
