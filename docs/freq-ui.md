...
AI Trading Bot provides a builtin webserver, which can serve [FreqUI](https://github.com/MuhammadMuazzain/ai-trading-bot), the ai-trading-bot frontend.

By default, the UI is automatically installed as part of the installation (script, docker).
freqUI can also be manually installed by using the `ai-trading-bot install-ui` command.
This same command can also be used to update freqUI to new new releases.

Once the bot is started in trade / dry-run mode (with `ai-trading-bot trade`) - the UI will be available under the configured API port (by default `http://127.0.0.1:8080`).

??? Note "Looking to contribute to freqUI?"
    Developers should not use this method, but instead clone the corresponding use the method described in the [freqUI repository](https://github.com/MuhammadMuazzain/ai-trading-bot) to get the source-code of freqUI. A working installation of node will be required to build the frontend.

!!! tip "freqUI is not required to run ai_trading_bot"
    freqUI is an optional component of ai_trading_bot, and is not required to run the bot.
    It is a frontend that can be used to monitor the bot and to interact with it - but ai-trading-bot itself will work perfectly fine without it.
...
When ai-trading-bot is started in [webserver mode](utils.md#webserver-mode) (ai-trading-bot started with `ai-trading-bot webserver`), the backtesting view becomes available.
