...
- the [AI Trading Bot strategy 101](strategy-101.md), which provides a quick start to strategy development
- the [AI Trading Bot bot basics](bot-basics.md), which provides overall info on how the bot operates

## Develop your own strategy

The bot includes a default strategy file.

Also, several other strategies are available in the [strategy repository](https://github.com/MuhammadMuazzain/ai-trading-bot-strategies).
...
```bash
ai-trading-bot new-strategy --strategy AwesomeStrategy
```
...
!!! Note
    There is a difference between the *name* of the strategy and the filename. In most commands, AI Trading Bot uses the *name* of the strategy, *not the filename*.
...
??? Hint "Different template levels"
    `ai-trading-bot new-strategy` has an additional parameter, `--template`, which controls the amount of pre-build information you get in the created strategy. Use `--template minimal` to get an empty strategy without any indicator examples, or `--template advanced` to get a template with more complicated features defined.
...
```bash
ai-trading-bot trade --strategy AwesomeStrategy
```

### Bot modes

AI Trading Bot strategies can be processed by the AI Trading Bot bot in 5 main modes:
...
## Diving in deeper
**For the following section we will use the [user_data/strategies/sample_strategy.py](https://github.com/MuhammadMuazzain/ai-trading-bot/blob/develop/ai_trading_bot/templates/sample_strategy.py)
file as reference.**
...
??? Hint "Lookahead and recursive analysis"
    AI Trading Bot includes two helpful commands to help assess common lookahead (using future data) and 
...
AI Trading Bot uses [pandas](https://pandas.pydata.org/) to store/provide the candlestick (OHLCV) data.
...
AI Trading Bot uses this new column as an entry signal, where it is assumed that a trade will subsequently open on the next open candle.
...
AI Trading Bot does not store incomplete/unfinished candles in the dataframe.
...
AI Trading Bot does not. Only complete/finished candle data is available in the dataframe.
...
!!! Note "Want more indicator examples?"
    Look into the [user_data/strategies/sample_strategy.py](https://github.com/MuhammadMuazzain/ai-trading-bot/blob/develop/ai_trading_bot/templates/sample_strategy.py).
    Then uncomment indicators you need.

#### Indicator libraries

Out of the box, ai-trading-bot installs the following technical libraries:

- [ta-lib](https://ta-lib.github.io/ta-lib-python/)
- [pandas-ta](https://twopirllc.github.io/pandas-ta/)
- [technical](https://github.com/MuhammadMuazzain/ai-trading-bot)

Additional technical libraries can be installed as necessary, or custom indicators may be written / invented by the strategy author.

### Strategy startup period

Some indicators have an unstable startup period in which there isn't enough candle data to calculate any values (NaN), or the calculation is incorrect. This can lead to inconsistencies, since AI Trading Bot does not know how long this unstable period is and uses whatever indicator values are in the dataframe.
...
!!! Warning "Using x calls to get OHLCV"
    ...
    Having this will cause AI Trading Bot to make multiple calls for the same pair, which will obviously be slower than one network request.
    As a consequence, AI Trading Bot will take longer to refresh candles - and should therefore be avoided if possible.
    This is capped to 5 total calls to avoid overloading the exchange, or make ai-trading-bot too slow.
...
``` bash
ai-trading-bot backtesting --timerange 20190101-20190201 --timeframe 5m
```
...
``` python
from ai_trading_bot.exchange import timeframe_to_minutes
...
By default, ai-trading-bot will attempt to load strategies from all `.py` files within the `userdir` (default `user_data/strategies`).

Assuming your strategy is called `AwesomeStrategy`, stored in the file `user_data/strategies/AwesomeStrategy.py`, then you can start ai-trading-bot in dry (or live, depending on your configuration) mode with:
...
You can use `ai-trading-bot list-strategies` to see a list of all strategies AI Trading Bot is able to load (all strategies in the correct folder).
...
??? Example "Fast and easy way to define informative pairs"
    ...
    from ai_trading_bot.strategy import IStrategy, informative
...
This method is used by ai-trading-bot internally to determine the last signal.
...
```python
from ai_trading_bot.strategy import IStrategy, merge_informative_pair
...
AI Trading Bot locks pairs automatically for the current candle (until that candle is over) when a pair exits, preventing an immediate re-entry of that pair.
...
AI Trading Bot has an easy method to do this from within the strategy, by calling `self.lock_pair(pair, until, [reason])`.
...
When conflicting signals collide (e.g. both `'enter_long'` and `'exit_long'` are set to `1`), ai-trading-bot will do nothing and ignore the entry signal. This will avoid trades that enter, and exit immediately. Obviously, this can potentially lead to missed entries.
...
To get additional ideas for strategies, head over to the [strategy repository](https://github.com/MuhammadMuazzain/ai-trading-bot-strategies). Feel free to use them as examples, but results will depend on the current market situation, pairs used, etc. Therefore, these strategies should be considered only for learning purposes, not real world trading. Please backtest the strategy for your exchange/desired pairs first, then dry run to evaluate carefully, and use at your own risk.
