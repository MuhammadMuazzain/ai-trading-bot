...
The ai-trading-bot documentation describes various ways to install ai_trading_bot
...
Please consider using the prebuilt [docker images](docker_quickstart.md) to get started quickly while evaluating how ai-trading-bot works.
...
The easiest way to install and run AI Trading Bot is to clone the bot Github repository and then run the `./setup.sh` script, if it's available for your platform.
...
!!! Note
    Python3.10 or higher and the corresponding `pip` are assumed to be available. The install-script will warn you and stop if that's not the case. `git` is also needed to clone the AI Trading Bot repository.  
...
!!! Note "ARM64 systems"
    If you are running an ARM64 system (like a MacOS M1 or an Oracle VM), please use [docker](docker_quickstart.md) to run ai_trading_bot.
...
=== "RaspberryPi/Raspbian"
    ...
    This image comes with python3.11 preinstalled, making it easy to get ai-trading-bot up and running.
...
## AI Trading Bot repository

AI Trading Bot is an open source crypto-currency trading bot, whose code is hosted on `github.com`

```bash
# Download `develop` branch of ai-trading-bot repository
git clone https://github.com/MuhammadMuazzain/ai-trading-bot.git
...
??? Note "Install from pypi"
    An alternative way to install AI Trading Bot is from [pypi](https://pypi.org/project/ai_trading_bot/). The downside is that this method requires ta-lib to be correctly installed beforehand, and is therefore currently not the recommended way to install AI Trading Bot.

    ``` bash
    pip install ai_trading_bot
    ```
...
First of the ways to install AI Trading Bot, is to use provided the Linux/MacOS `./setup.sh` script, which install all dependencies and help you configure the bot.

Make sure you fulfill the [Requirements](#requirements) and have downloaded the [AI Trading Bot repository](#ai_trading_bot-repository).

### Use /setup.sh -install (Linux/MacOS)

If you are on Debian, Ubuntu or MacOS, ai-trading-bot provides the script to install ai_trading_bot.

```bash
# --install, Install ai-trading-bot from scratch
./setup.sh -i
```
...
You will run ai-trading-bot in separated `virtual environment`

```bash
# create virtualenv in directory /ai_trading_bot/.venv
python3 -m venv .venv
...
AI Trading Bot can also be installed with Miniconda or Anaconda. We recommend using Miniconda as it's installation footprint is smaller. Conda will automatically prepare and manage the extensive library-dependencies of the AI Trading Bot program.
...
#### AI Trading Bot download

Download and install ai_trading_bot.

```bash
# download ai_trading_bot
git clone https://github.com/MuhammadMuazzain/ai-trading-bot.git

# enter downloaded directory 'ai-trading-bot'
cd ai-trading-bot      
```

#### AI Trading Bot install: Conda Environment

```bash
conda create --name ai-trading-bot python=3.12
```
...
#### Enter/exit ai-trading-bot environment
...
# activate ai-trading-bot environment
conda activate ai_trading_bot
...
You've made it this far, so you have successfully installed ai_trading_bot.
...
ai-trading-bot new-config --config user_data/config.json
```
...
```bash
ai-trading-bot trade --config user_data/config.json --strategy SampleStrategy
```
...
```bash
# if:
bash: ai_trading_bot: command not found
