...
If you're just getting started, please familiarize yourself with the [AI Trading Bot basics](bot-basics.md) and methods described in [Strategy Customization](strategy-customization.md) first.
...
!!! Tip
    Start off with a strategy template containing all available callback methods by running `ai-trading-bot new-strategy --strategy MyAwesomeStrategy --template advanced`

## Storing information (Persistent)

AI Trading Bot allows storing/retrieving user custom information associated with a specific trade in the database.
...
For the data to be able to be stored within the database, ai-trading-bot must serialized the data. This is done by converting the data to a JSON formatted string.
AI Trading Bot will attempt to reverse this action on retrieval, so from a strategy perspective, this should not be relevant.
...
``` python
from ai_trading_bot.exchange import timeframe_to_prev_date
...
!!! Warning
    ...
    These results are a consequence of the strategy overwriting prior tags - where the last tag will "stick" and will be the one ai-trading-bot will use.
...
!!! Note
    You should make sure to implement proper version control (like a git repository) alongside this, as ai-trading-bot will not keep historic versions of your strategy, so it's up to the user to be able to eventually roll back to a prior version of the strategy.
...
AI Trading Bot provides you with an easy way to embed the strategy into your configuration file.
...
AI Trading Bot does however also counter this by running `dataframe.copy()` on the dataframe right after the `populate_indicators()` method - so performance implications of this should be low to non-existent.
