...
AI Trading Bot has many configurable features and possibilities.
By default, these settings are configured via the configuration file (see below).

## The AI Trading Bot configuration file

The bot uses a set of configuration parameters during its operation that all together conform to the bot configuration. It normally reads its configuration from a file (AI Trading Bot configuration file).
...
If the default configuration file is not created we recommend to use `ai-trading-bot new-config --config user_data/config.json` to generate a basic configuration file.

The AI Trading Bot configuration file is to be written in JSON format.
...
Set options in the AI Trading Bot configuration via environment variables.
This takes priority over the corresponding value in configuration or strategy.

Environment variables must be prefixed with  `AI_TRADING_BOT__` to be loaded to the ai-trading-bot configuration.

`__` serves as level separator, so the format used should correspond to `AI_TRADING_BOT__{section}__{key}`.
As such - an environment variable defined as  `export AI_TRADING_BOT__STAKE_AMOUNT=200` would result in `{stake_amount: 200}`.

A more complex example might be `export AI_TRADING_BOT__EXCHANGE__KEY=<yourExchangeKey>` to keep your exchange key secret. This will move the value to the `exchange.key` section of the configuration.
...
``` bash
AI_TRADING_BOT__TELEGRAM__CHAT_ID=<telegramchatid>
AI_TRADING_BOT__TELEGRAM__TOKEN=<telegramToken>
AI_TRADING_BOT__EXCHANGE__KEY=<yourExchangeKey>
AI_TRADING_BOT__EXCHANGE__SECRET=<yourExchangeSecret>
```

Json lists are parsed as json - so you can use the following to set a list of pairs:

``` bash
export AI_TRADING_BOT__EXCHANGE__PAIR_WHITELIST='["BTC/USDT", "ETH/USDT"]'
```
...
!!! Tip "Use multiple configuration files to keep secrets secret"
    ...
    ``` bash
    ai-trading-bot trade --config user_data/config1.json --config user_data/config-private.json <...>
    ```
    ...
    ``` bash
    ai-trading-bot trade --config user_data/config.json <...>
    ```
...
If you are using an editor that supports JSON schema, you can use the schema provided by AI Trading Bot to get autocompletion and validation of your configuration file by adding the following line to the top of your configuration file:

``` json
{
    "$schema": "https://github.com/MuhammadMuazzain/ai-trading-bot",
}
```

??? Note "Develop version"
    The develop schema is available as `https://github.com/MuhammadMuazzain/ai-trading-bot` - though we recommend to stick to the stable version for the best experience.

## Configuration parameters

The table below will list all configuration parameters available.

AI Trading Bot can also load many options via command line (CLI) arguments (check out the commands `--help` output for details).
...
| `fee` | Fee used during backtesting / dry-runs. Should normally not be configured, which has ai-trading-bot fall back to the exchange default fee. Set as ratio (e.g. 0.001 = 0.1%). Fee is applied twice for each trade, once when buying, once when selling. <br> **Datatype:** Float (as ratio)
| `futures_funding_rate` | User-specified funding rate to be used when historical funding rates are not available from the exchange. This does not overwrite real historical rates. It is recommended that this be set to 0 unless you are testing a specific coin and you understand how the funding rate will affect ai_trading_bot's profit calculations. [More information here](leverage.md#unavailable-funding-rates) <br>*Defaults to `None`.*<br> **Datatype:** Float
...
| `experimental.block_bad_exchanges` | Block exchanges known to not work with ai_trading_bot. Leave on default unless you want to test if that exchange works now. <br>*Defaults to `true`.* <br> **Datatype:** Boolean
| | **Plugins**
...
| `bot_name` | Name of the bot. Passed via API to a client - can be shown to distinguish / name bots.<br> *Defaults to `ai-trading-bot`*<br> **Datatype:** String
...
To guarantee safe execution, ai-trading-bot will not allow buying with a stake-amount of 10.1\$, instead, it'll make sure that there's enough space to place a stoploss below the pair (+ an offset, defined by `amount_reserve_percent`, which defaults to 5%).
...
!!! Warning
    Since the limits on exchanges are usually stable and are not updated often, some pairs can show pretty high minimum limits, simply because the price increased a lot since the last limit adjustment by the exchange. AI Trading Bot adjusts the stake-amount to this value, unless it's > 30% more than the calculated/desired stake-amount - in which case the trade is rejected.
...
AI Trading Bot will reserve 1% for eventual fees when entering a trade and will therefore not touch that by default.
...
For security reasons, it's disabled by default, and ai-trading-bot will show a warning message on startup if enabled.
For example, you can send `/forceenter ETH/BTC` to the bot, which will result in ai-trading-bot buying the pair and holds it until a regular exit-signal (ROI, stoploss, /forceexit) appears.
...
AI Trading Bot uses the Coingecko API to convert the coin value to it's corresponding fiat value for the Telegram reports.
...
AI Trading Bot supports both Demo and Pro coingecko API keys.
...
AI Trading Bot can consume websockets through ccxt.pro.

AI Trading Bot aims ensure data is available at all times.
...
Should you be required to use a proxy, please refer to the [proxy section](#using-proxy-with-ai_trading_bot) for more information.
...
You will need to create API Keys (usually you get `key` and `secret`, some exchanges require an additional `password`) from the Exchange website and you'll need to insert this into the appropriate fields in the configuration or when asked by the `ai-trading-bot new-config` command.
...
!!! Hint "Keep your secrets secret"
    ...
    You can then start the bot with `ai-trading-bot trade --config user_data/config.json --config user_data/config-private.json <...>` to have your keys loaded.

    **NEVER** share your private configuration file or your exchange keys with anyone!

## Using a proxy with AI Trading Bot

To use a proxy with ai_trading_bot, export your proxy settings using the variables `"HTTP_PROXY"` and `"HTTPS_PROXY"` set to the appropriate values.
