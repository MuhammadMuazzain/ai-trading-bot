...
from ai_trading_bot_client.cli import add_arguments, main_exec
...
def get_rest_client():
    client = RestApiClient("http://localhost:8080", "ai_trading_botr", "password")
...
def test_RestApiClient_init():
    client = RestApiClient("http://localhost:8080", "ai_trading_botr", "password")
    assert client is not None
    assert client._serverurl == "http://localhost:8080"
    assert client._session is not None
    assert client._session.auth is not None
    assert client._session.auth == ("ai_trading_botr", "password")


@pytest.mark.parametrize("method", ["GET", "POST", "DELETE"])
def test_RestApiClient_call(method):
...
def test_RestApiClient_call_invalid(caplog):
...
def test_RestApiClient_call_explicit_methods(method, args, kwargs):
    client, mock = get_rest_client()
...
def test_rest_client(mocker, capsys, caplog):
    ...
    mock = mocker.patch("ai_trading_bot_client.cli.RestApiClient._call")
...
def test_rest_client_argparsing(mocker, params, expected_args, expected_kwargs, caplog):
    mocked_method = params.split(" ")[0]
    mocker.patch("ai_trading_bot_client.cli.load_config", return_value={}, autospec=True)
    mm = mocker.patch(
        f"ai_trading_bot_client.cli.RestApiClient.{mocked_method}", return_value={}, autospec=True
    )
